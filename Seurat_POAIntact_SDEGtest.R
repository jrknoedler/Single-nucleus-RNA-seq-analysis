#!/usr/bin/env Rscript
output <- "Seurat/POA_IndependentAnalysis/IntactSDEG_Plottest"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
mySeurat <- readRDS("Seurat/POA_IndependentAnalysis/POA_IndependentFiltered7_30pcsredo_res1_Intact.rds")
head(mySeurat[[]])
genelist <- read.table("Genelists/POA_Allcomps_new.txt")
unlist(genelist)
...
Upmale <- read.table("Genelists/POA_AllupMale_New.txt")
unlist(Upmale)
