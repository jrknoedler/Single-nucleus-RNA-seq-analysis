...
output <- "Seurat/BNST_IndependentAnalysis/BNST_Fullmerge_Test_prepaperreclusterRound5_filtered5alt"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
mySeurat <- readRDS("Seurat/BNST_IndependentAnalysis/BNST_Fullmerge_Test_prepaperreclusterRound5_filtered3.rds")
#DefaultAssay(mySeurat) <- "integrated"
mySeurat <- subset(mySeurat, idents=c("28","36"), invert=TRUE)
...
hvg <- unlist(hvg)
hvg
write.csv(hvg, file=paste0(output,"BNSTvarft.csv"))
hvg <- as.matrix(hvg)
...
pdf(paste0(output,"_TopMarkerheatmap.pdf"))#top10 <- mySeurat.markers %>% group_by(cluster) %>% top_n(n=10, wt=avg_logFC)
