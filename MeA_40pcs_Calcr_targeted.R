...
Primed$sex <- "Primed"
Male$sex <- "Male" 
Primed <- subset(Primed, idents=c("15"))
Male <- subset(Male, idents=c("18"))
...
markers <- FindMarkers(mySeurat, ident.1="Primed", ident.2="Male", min.pct=0, logfc.threshold=0)
write.csv(markers, file="Seurat/MeA_IndependentAnalysis/MeA_Calcr_res1_40pcs_MvP_SDEGsdefault.csv")