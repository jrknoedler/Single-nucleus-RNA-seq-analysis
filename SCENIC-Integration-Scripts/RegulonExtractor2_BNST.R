...
output <- "RegulonsCompiled/BNST/BNST_Unfiltered"
listoutput <- "RegulonsCompiled/BNST/RegulonDegTargets/"

loom <- open_loom("pySCENIC_Singularity/BNST_DefaultNES_Final/auc_mtx_filtered.loom") 
regulons_incidMat <- get_regulons(loom, attrName="Regulons")
regulons <- regulonsToGeneLists(regulons_incidMat)

loom <- close_loom()

MvUDEGS <- read.table("Genelists/BNST_MvU_1.5cutoff.txt")
MvUDEGS <- unlist(MvUDEGS)
MvUDEGS <- as.matrix(MvUDEGS)

PvUDEGS <- read.table("Genelists/BNST_PvU_1.5cutoff.txt")
PvUDEGS <- unlist(PvUDEGS)
PvUDEGS <- as.matrix(PvUDEGS)

MvPDEGS <- read.table("Genelists/BNST_MvP_1.5cutoff.txt")
...
c("Tcf4(+)", "Otp(+)","Arx(+)","Cux1(+)","Nrf1(+)")

tcf4 <- "Tcf(+)"
otp <- "Otp(+)"
Arx <- "Arx(+)"

df <- data.frame(regulons$Arx)
df
DEGsReg <- intersect(df, AllDegs)
write.csv(df, file="Arxpleasework.csv")

tcf4 <- regulons$Tcf4
tcf4
tcf42 <- regulons$Tcf4(+)
tcf42
write.csv(tcf4, file="Fuckthistcf.csv")
otp <- regulons$Otp
write.csv(otp, file="Fuckthisotp.csv")
Arx <- regulons$Arx
write.csv(Arx, file="Fuckthisarx.csv")
cux1 <- regulons$Cux1
write.csv(cux1, file="Fuckthiscux1.csv")
nrf1 <- regulons$Nrf1
write.csv(nrf1, file="Fuckthisnrf1.csv")
...
for (i in regulons){
regulon <- names(i)
regulon <- unlist(regulon)
dummy <- cbind(c(regulon, 0))
df <- data.frame(i)
total <- nrow(df)
df <- as.matrix(df)
regname <- colnames(i)
DEGsReg <- intersect(df, AllDegs)
genelist <- rbind(dummy, DEGsReg)
genelist <- data.frame(genelist)
DEGsReg <- data.frame(DEGsReg)
frac <- nrow(DEGsReg)
df2 <- data.frame(frac, total)
master_list4=rbind(master_list4, df2)
explist=rbind(explist, genelist)
}

final4 <- cbind(master_list4, regs)
write.csv(final4, file=paste0(output,"regulonoverlapallVMHDEGs.csv"))
write.csv(explist,file=paste0(output,"Regulongenetaragets.csv"))
