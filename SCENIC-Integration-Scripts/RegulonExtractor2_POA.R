...
output <- "RegulonsCompiled/POA/POA_Unfiltered"

loom <- open_loom("pySCENIC_Singularity/POA_DefaultNES_Final/auc_mtx_filtered.loom") 
regulons_incidMat <- get_regulons(loom, attrName="Regulons")
regulons <- regulonsToGeneLists(regulons_incidMat)



MvUDEGS <- read.table("Genelists/POA_MvU_1.5cutoff.txt")
MvUDEGS <- unlist(MvUDEGS)
MvUDEGS <- as.matrix(MvUDEGS)

PvUDEGS <- read.table("Genelists/POA_PvU_1.5cutoff.txt")
PvUDEGS <- unlist(PvUDEGS)
PvUDEGS <- as.matrix(PvUDEGS)

MvPDEGS <- read.table("Genelists/POA_MvP_1.5cutoff.txt")
