...
output <- "Seurat/MeA_IndependentAnalysis/MeA_Fullmerge_20pcs_0.1cutoff_1.5cutoff"

mySeurat <- readRDS("Seurat/MeA_IndependentAnalysis/MeA_Fullmerge_Test_20pcs.rds")
genes.10x <- (x=rownames(x=mySeurat))
unlist(genes.10x)
MvPsdegs <- read.table("Genelists/MeA_MvP_1.5cutoff.txt")
unlist(MvPsdegs)
MvPsdegs <- as.matrix(MvPsdegs)
MvPsdegs.filtered <- intersect(MvPsdegs, genes.10x)
MvUsdegs <- read.table("Genelists/MeA_MvU_1.5cutoff.txt")
unlist(MvUsdegs)
MvUsdegs <- as.matrix(MvUsdegs)
MvUsdegs.filtered <- intersect(MvUsdegs, genes.10x)
PvUsdegs <- read.table("Genelists/MeA_PvU_1.5cutoff.txt")
unlist(PvUsdegs)
