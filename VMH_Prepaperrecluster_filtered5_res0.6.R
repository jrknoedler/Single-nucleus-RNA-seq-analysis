...
output <- "Seurat/VMH_IndependentAnalysis/VMH_Fullmerge_Test_prepaperreclusterRound2_sexclude_malat1regress_filtered5_30pcsres0.6"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
mySeurat <- readRDS("Seurat/VMH_IndependentAnalysis/VMH_Fullmerge_Test_prepaperreclusterRound2_sexclude_malat1regress_filtered4_20pcsres1.rds")
#DefaultAssay(mySeurat) <- "integrated"
mySeurat <- subset(mySeurat, idents=c("17","15","20"), invert=TRUE)
...
VlnPlot(mySeurat, features=c("Esr1","Pgr","Slc17a6","Gad1","Cyp19a1","Cckar","Myo1h"), pt.size=0, ncol=1)
dev.off()
