...
output <- "Seurat/VMH_IndependentAnalysis/VMH_CelltypesDraftArcfiltered_regressed_reorderedJune_unfiltered"
...
VMHdots <- VMHdata  %>% ggplot(aes(x=features.plot, y=factor(id, levels=VMHlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_cowplot() +
  geom_point() + 
  scale_colour_gradientn(colours = c("blue","red"), limits=c(-1.5,2.5))+ scale_size(limits=c(0,100),breaks=seq(25,100,25)) +
...
MeAdots <- MeAdata  %>% ggplot(aes(x=features.plot, y=factor(id, levels=MeAlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_cowplot() +
  geom_point() + 
...
POAdots <- POAdata  %>% ggplot(aes(x=features.plot, y=factor(id, levels=POAlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_cowplot() +
  geom_point() + 
  scale_colour_gradientn(colours = c("blue","red"), limits=c(-1.5,2.5))+ scale_size(limits=c(0,100)) +
  theme(axis.line  = element_blank())   + theme(axis.title.x=element_blank()) +
  theme(axis.text.x = element_blank(), axis.text.y=element_text(hjust=1)) +
...
BNSTdots <- BNSTdata %>% ggplot(aes(x=features.plot, y=factor(id, levels=BNSTlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_cowplot() +
  geom_point() + 
  scale_colour_gradientn(colours = c("blue","red"), limits=c(-1.5,2.5))+ scale_size(limits=c(0,100)) +
  theme(axis.line  = element_blank())   + theme(axis.title.x=element_blank()) +
  theme(axis.text.x = element_blank(), axis.text.y=element_text(hjust=1)) +
...
pdf(file="Seurat/Fullfig4_checkscales_unfiltered.pdf", width=82, height=8)
