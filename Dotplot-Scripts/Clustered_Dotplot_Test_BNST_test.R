...
output <- "Seurat/BNST_IndependentAnalysis/BNST_CelltypesDraft_Malat1regress_Esr1filtered_sexclude_res1.2FINAL"
...
Pct <- DotPlot(mySeurat, features=typemarkers, slot="scale.data")
...
data <- Pct$data
head(data)
pdf(file=paste0(output,"_filtereddotplot.pdf"), width=16)
data %>% 
  filter(pct.exp > 25) %>% ggplot(aes(x=features.plot, y=factor(id, levels=rev(levels(factor(id)))), color=avg.exp, size=pct.exp)) + 
  geom_point() + scale_color_gradientn(colors=viridis(1000), limits=c(0,20)) +
