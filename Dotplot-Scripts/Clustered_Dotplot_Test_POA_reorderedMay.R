...
output <- "Seurat/POA_IndependentAnalysis/POA_CelltypesKiss1regressed_oldorderMay_"
...
mySeurat <- readRDS("Seurat/POA_IndependentAnalysis/POA_Fullmerge_Kiss1opt_Filtered3_30pcs_res1.2_malatregressexclude.rds")
...
typemarkers <- read.table("DotPlotMarkerLists/POA_Kiss1opt2regressed.txt")
typemarkers <- unlist(typemarkers)
a <- AverageExpression(mySeurat, features=typemarkers)
a2 <- a$RNA
a3 <- scale(t(a2))
write.csv(a3, file=paste0(output,"scaledclustermarkers.csv"))
pdf(file=paste0(output,"scaledclustermarkerheatmap.pdf"))
pheatmap(a3, cluster_rows=FALSE, cluster_cols=FALSE, color=magma(2000))
dev.off()
typemarkers <- as.matrix(typemarkers)

mySeurat <- ScaleData(mySeurat)
mylevels=c(0,1,5,6,10,12,16,22,24,26,32,2,3,4,7,8,9,11,13,14,15,17,18,19,20,21,23,25,27,28,29,30,31)
