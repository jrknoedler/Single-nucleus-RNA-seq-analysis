...
output <- "Seurat/MeA_IndependentAnalysis/MeA_CelltypesCCAfiltered1Mayreviseredo"
...
mySeurat <- readRDS("Seurat/MeA_IndependentAnalysis/MeA_CCAfiltered1_res1.2marredo_esr1filt_33filt2_0.9.rds")
...
VlnPlot(mySeurat, idents=c("21"), features=rev(c("Gad1","Slc17a6","Slc18a2","Frmpd1","Lrp4","Kiss1")), cols=c("purple"), ncol=1, pt.size=0)
dev.off()
pdf(file=paste0(output,"EstrusClustMarkers.pdf"), height=20)
VlnPlot(mySeurat, idents=c("5","22","28"), features=rev(c("Gad1","Slc17a6","Isl1","Ebf3","Glra1","Lmo7","Abtb2","Cckar","Myo1h")), cols=c("yellow","dark green","blue"), ncol=1, pt.size=0)
dev.off()
...
VlnPlot(mySeurat, features=c("Nfia","Nfix","Auts2","Nr3c2"), pt.size=0, split.by="Hormone", ncol=1)
dev.off()
pdf(file=paste0(output,"biasmarkers.pdf"), width=10, height=20)
VlnPlot(mySeurat, idents=c("5","9","21","22","30"), features=rev(c("Isl1","Vgll3","Frmpd1","Ebf3","Pappa2")), ncol=1, pt.size=0)
dev.off()
...
dev.off() 
typemarkers <- read.table("DotPlotMarkerLists/MeAReorganizedFiltered_marchredo.txt")
...
Pct$data
sink()
mylevels <- c(0,3,8,12,13,14,16,17,20,21,22,28,30,31,32,33,1,2,5,6,7,10,11,15,18,19,23,24,26,27,29,30,4,9,25)
mylevels <- rev(levels)
data <- Pct$data
head(data)
