...
data %>% filter(pct.exp > 10) %>% ggplot(aes(x=id, y=features.plot, color=avg.exp, size=pct.exp)) + geom_point() + theme(axis.text.x = element_text(theme(axis.ticks=element_blank()) + scale_color_gradientn(colors=viridis::viridis(> dev.off()=c(0,30), oob=scales::squish)
dev.off()
