...
output <- "Seurat/BNST_IndependentAnalysis/BNST_CelltypesDraft_Malat1regress_Esr1filtered_sexclude_res1.2alt2"
...
byCyp19a1 <- data %>% filter(features.plot=="Cyp19a1")
sortedfeature <- byCyp19a1[order(-byCyp19a1$avg.exp.scaled),]
byTac1 <- data %>% filter(id==17)   
sorted <- byTac1[order(-byTac1$pct.exp),]

pdf(file=paste0(output,"_filtereddotplot_SFARI.pdf"), width=16)
data %>% 
  filter(pct.exp > 25) %>% ggplot(aes(x=(factor(features.plot,levels=sorted$features.plot)), y=factor(id, levels=rev(sortedfeature$id)), color=avg.exp.scaled, size=pct.exp)) + 
