...
output <- "Seurat/PrimedPOA_BusExploratory/PrimedPOA_emcounts_kiss"
...
library(ggplot2)
library(dplyr)
Seurat.data <- read_count_output("Kallisto_Bus_Output/PrimedPOA/combined_counts_em/", name="output", tcc=FALSE)
mySeurat <- CreateSeuratObject(counts = Seurat.data, project = sampleID, min.cells=0, min.features=200)
mySeurat
...
dev.off()
pdf(paste0(output, "_Kiss1.pdf"))
VlnPlot(mySeurat, features = c("Kiss1"))
dev.off()
