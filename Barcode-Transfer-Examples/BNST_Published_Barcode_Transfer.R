...
#sampleID <- "PregPOA"
output <- "Seurat/BNST_Barcode_Transfer/BNST_Barcode_Lift"
...
library(dplyr)
library(Matrix)
Primed.data <- read_count_output("Updated_Ref_Kb_Out/BNST_Primed/output_mtx_em", name="output", tcc=FALSE)
Unprimed.data <- read_count_output("Updated_Ref_Kb_Out/BNST_Unprimed/output_mtx_em", name="output", tcc=FALSE)
Male.data <- read_count_output("Updated_Ref_Kb_Out/Male_BNST/output_mtx_em", name="output", tcc=FALSE)
Primed.barcodes <- read.table("Barcodes_For_Transfer/Primed_BNST_Barcodes.txt", header=FALSE)
Primed.barcodes <- unlist(Primed.barcodes)
Unprimed.barcodes <- read.table("Barcodes_For_Transfer/Unprimed_BNST_Barcodes.txt", header=FALSE)
Unprimed.barcodes <- unlist(Unprimed.barcodes)
Male.barcodes <- read.table("Barcodes_For_Transfer/Male_BNST_Barcodes.txt", header=FALSE)
...
PrimedSeurat <- CreateSeuratObject(counts = Primed.filtered, project = "PrimedBNST", min.cells=3, min.features=500)
UnprimedSeurat <- CreateSeuratObject(counts = Unprimed.filtered, project = "UnprimedBNST", min.cells=3, min.features=500)
MaleSeurat <- CreateSeuratObject(counts=Male.filtered, project = "MaleBNST", min.cells=3, min.features=500)
...
Pregnant <- readRDS("/scratch/groups/nirao/shared1/Preg_BNST_QCFiltered.rds")
Pregnant$sex <- "Female"
Pregnant$Hormone <- "Pregnant"
mySeurat <- merge(x=Pregnant, y=c(PrimedSeurat,UnprimedSeurat,MaleSeurat), add.cell.ids=c("Pregnant","Primed","Unprimed","Male"), project="BNST_Preg_Merged")
