...
BNSTgenes <- c("Rbm20","Plekhg1","Cck","Phf21b","Tac1","Asic4","Kank1","Sst","Abca1","Amot","Prkcd","Sertm1","Tns1","Sytl4","Npr3","Plscr4")
MeAgenes <- c("Vgll3","Efemp1")
POAgenes <- c("Moxd1","St3gal1","Mrc2","Ttn","Neb","Kiss1","Slc17a8","Abtb2","Penk")
...
plotf <- plot & theme(axis.text.x=element_blank(), axis.text.y=element_blank(),axis.ticks=element_blank(), axis.title=element_blank(), plot.title=element_blank()) & geom_violin(trim=TRUE, fill="red", scale="width", color="light gray") 
...
pdf(file="Seurat/MeA_SpecDEGVLnnew.pdf", width=37, height=3.75)
...
ggplot(POAout, aes(y=factor(POAgenes, levels=POAgenes), x=CPM, color=log10(max))) +cowplot::theme_cowplot() + geom_point(size=20, breaks=c(0)) + scale_color_viridis() +  theme(axis.line  = element_blank())   + theme(axis.title=element_blank()) + labs(x="") + theme(axis.text.x = element_blank()) + theme(axis.ticks = element_blank())
dev.off()

plot <- VlnPlot(POA, features=rev(POAgenes), pt.size=0, ncol=1)
plotf <- plot & theme(axis.text.x=element_blank(), axis.text.y=element_blank(),axis.ticks=element_blank(), axis.title=element_blank(), plot.title=element_blank()) & geom_violin(trim=TRUE, fill="red", scale="width", color="light gray")
pdf(file="Seurat/POA_Dotplot")
DotPlot(POA, features=POAgenes)
dev.off()
pdf(file="Seurat/POA_SpecDEGVLnnew.pdf", width=37, height=16)
