...
mySeurat <- readRDS("/scratch/users/knoedler/Seurat/POA_IndependentAnalysis/POA_Fullmerge_Kiss1opt_Filtered3_40pcs_res1.2.rds")

regulons <- read.csv("/scratch/users/knoedler/pySCENIC_Singularity/POAMerged/auc_mtx_filtered.csv", header=TRUE, row.names=1)
dim(regulons)
...
exp <- scale((exp))
idents
row.names(exp) <- c(0:42)
exp <- data.frame(exp)
pdf(file="POA_regbyclust.pdf", width=40, height=12)
...
topplot <- exp[,topregs]
pdf(file="POA_topregsbyclust.pdf", width=30, height=12)
pheatmap(topplot, show_rownames=T, border_color=NA, color=viridis(500))
dev.off()
RegDimorphic <- read.table("RegulonsCompiled/POA/DegulonsSeurat.txt", header=FALSE)
RegDimorphic <- unlist(RegDimorphic)
RegDimorphic <- as.matrix(RegDimorphic)
RegDimorphic
expdimorphic <- exp[,RegDimorphic]

pdf(file="POA_regbyclustdimorphic.pdf", width=20, height=12)
