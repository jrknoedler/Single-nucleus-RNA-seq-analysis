...
output <- "Scrattch_hicat/MeA/MeAMerged_Paperanalysis_combinat"
...
mySeurat <- readRDS("Seurat/MeA_IndependentAnalysis/MeA_Fullmerge_Test_SCT.rds")
cells <- rownames(mySeurat[[]])
data <- mySeurat@assays[["RNA"]]@counts
data <- as.matrix(data)
de.param <- de_param(padj.th = 0.05, lfc.th=0.5, low.th=1, q1.th=0.3, q2.th=NULL, q.diff.th=0.1, de.score.th=100, min.cells=10)
data <- cpm(data)
...
onestep.result <- onestep_clust(norm.data, dim.method="pca", de.param=strict.param, rm.eigen=rm.eigen)
...
merge.param1 <- de_param(de.score.th=200)
