...
output <- "Seurat/POA_IndependentAnalysis/POA_Fullmerge_30pcs_0.1_1.5cutoff"

mySeurat <- readRDS("Seurat/POA_IndependentAnalysis/POA_Fullmerge_Test.rds")
genes.10x <- (x=rownames(x=mySeurat))
unlist(genes.10x)
MvPsdegs <- read.table("Genelists/POA_MvP_1.5cutoff.txt")
unlist(MvPsdegs)
MvPsdegs <- as.matrix(MvPsdegs)
MvPsdegs.filtered <- intersect(MvPsdegs, genes.10x)
MvUsdegs <- read.table("Genelists/POA_MvU_1.5cutoff.txt")
unlist(MvUsdegs)
MvUsdegs <- as.matrix(MvUsdegs)
MvUsdegs.filtered <- intersect(MvUsdegs, genes.10x)
PvUsdegs <- read.table("Genelists/POA_PvU_1.5cutoff.txt")
unlist(PvUsdegs)
