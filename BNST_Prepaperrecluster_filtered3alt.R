...
output <- "Seurat/BNST_IndependentAnalysis/BNST_Fullmerge_Test_prepaperreclusterRound4_sexclude_malat1exclude_malat1regress_filtered225pcsres0.8"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
mySeurat <- readRDS("Seurat/BNST_IndependentAnalysis/BNST_Fullmerge_Test_prepaperreclusterRound4_sexclude_malat1exclude_malat1regress_filtered2.rds")
#DefaultAssay(mySeurat) <- "integrated"
mySeurat <- subset(mySeurat, idents=c("39"), invert=TRUE)
...
warnings()
mySeurat <- FindNeighbors(mySeurat, dims=1:25)
mySeurat <- FindClusters(mySeurat, resolution=0.8)
mySeurat <- RunUMAP(mySeurat, dims=1:25)
