output <- "Seurat/MeA_IndependentAnalysis/Paper_DraftAnalysis/RegressDegome_LessFilt/allornone_PvU"
...
files <- list.files(path="Seurat/MeA_IndependentAnalysis/Paper_DraftAnalysis/RegressDegome_LessFilt/PvU_Sig/", pattern="*.csv", full.names=TRUE, include.dirs=FALSE, recursive=FALSE)
...
Sig <- Sig[abs(Sig$log2FoldChange >=2),]
Sig <- data.frame(Sig)
...
write.table(master_list, file=paste0(output,"PvU.txt"))
warnings()
