...
files <- list.files(path="Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/RegressDegome/MvP_Sig/", pattern="*adjust.csv", full.names=TRUE, include.dirs=FALSE, recursive=FALSE)
...
DEGs <- na.omit(DEGs)
Sig1 <- DEGs[DEGs$padj < 0.05,]
Sig1 <- data.frame(Sig1)
Sigpos <- Sig1[Sig1$log2FoldChange >0,]
Top1pos <- Sigpos %>% top_n(1, log2FoldChange)
genes <- rownames(Top1pos)
genes <- unlist(genes)
genes <- data.frame(genes)
neg <- Sig1[Sig1$log2FoldChange < 0,]
Top1neg <- neg %>% top_n(1, abs(log2FoldChange))
genes1 <- rownames(Top1neg)
genes1 <- unlist(genes1)
genes1 <- data.frame(genes1)
genescomp <- rbind(genes, genes1)
master_list=rbind(master_list,genescomp)
})
master_list <- unlist(master_list)
master_list <- unique(master_list)
write.table(master_list, file=paste0(output,"MvPall.txt"))
warnings()
