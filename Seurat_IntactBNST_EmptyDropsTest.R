...
output <- "Seurat/BNST_IndependentAnalysis/Intact_EmptyDrops"
...
library(ggplot2)
library(dplyr)
#Seurat.data <- read_count_output("Kallisto_Bus_Output/IntactBNST/combined_counts_em/", name="output", tcc=FALSE)
#!/usr/bin/env Rscript
...
mtx.data <- read_count_output("Kallisto_Bus_Output/IntactBNST/combined_counts_em/", name="output", tcc=FALSE)
head(mtx.data)
set.seed(100)
e.out <- emptyDrops(mtx.data, lower=100, niter=50000)
e.out
...
filtered.counts <- mtx.data[,which(is.cell), drop=FALSE]
dim(filtered.counts)
mySeurat <- CreateSeuratObject(counts = filtered.counts, project = sampleID, min.cells=3, min.features=200)
mySeurat
...
dev.off()
mySeurat <- subset(mySeurat, subset=nCount_RNA < 100000)
