...
output <- "Seurat/BNST_IndependentAnalysis/BNSTMerged_DEGclust"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
hvg.final
DEGs <- read.table("topGO/Total_ByRegion/BNST_Genesonly.txt")
DEGs <- unlist(DEGs)
DEGs <- as.matrix(DEGs)
DEGs.filtered <- intersect(DEGs, genes.10x)
DEGs.filtered <- as.matrix(DEGs.filtered)
mySeurat <- RunPCA(mySeurat, features=DEGs.filtered, assay="SCT")
warnings()
...
pdf(file=paste0(output,"Cyp19a1Tac1lpdf"), width=25)
VlnPlot(mySeurat, feature=c("Cyp19a1","Tac1"), ncol=1, pt.size=0)
dev.off()
