...
library(tidyverse)
library(dplyr)
output <- "Seurat/VMH_IndependentAnalysis/VMH_Fullmerge_20pcs_0.1cutoff_1.5fold"

mySeurat <- readRDS("Seurat/VMH_IndependentAnalysis/VMHMerged_filtered2_Paperanalysis_20pcs.rds")
genes.10x <- (x=rownames(x=mySeurat))
unlist(genes.10x)
MvPsdegs <- read.table("Genelists/VMH_MvP_1.5.txt")
unlist(MvPsdegs)
MvPsdegs <- as.matrix(MvPsdegs)
MvPsdegs.filtered <- intersect(MvPsdegs, genes.10x)
MvUsdegs <- read.table("Genelists/VMH_MvU_1.5.txt")
unlist(MvUsdegs)
MvUsdegs <- as.matrix(MvUsdegs)
MvUsdegs.filtered <- intersect(MvUsdegs, genes.10x)
PvUsdegs <- read.table("Genelists/VMH_PvU_1.5.txt")
unlist(PvUsdegs)
...
dev.off()
pdf(file=paste0(output,"UMI.pdf"))
VlnPlot(mySeurat, features=c("nFeature_RNA"), pt.size=0, ncol=1)
dev.off()
pdf(file=paste0(output,"markers.pdf"))
VlnPlot(mySeurat, features=c("Slc23a1","Gad2","Slc17a6"), pt.size=0, ncol=1)
dev.off()
