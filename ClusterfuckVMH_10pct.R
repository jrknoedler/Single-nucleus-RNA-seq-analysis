...
output <- "Seurat/VMH_IndependentAnalysis/VMH_Merged_Clusterfuck_MayFinal_10pct_"
...
head(data.df)
pct <- data.df %>% group_by(id) %>% summarize(n_cells=n(), num=sum(pct.exp > 10), mvpprop=num/n_cells)
pct
...
head(data.df)
pct <- data.df %>% group_by(id) %>% summarize(n_cells=n(), num=sum(pct.exp > 10), pvuprop=num/n_cells)
pct
write.csv(pct, file=paste0(output,"PvU_sDEGspercluster.csv"))
dist <- data.df %>% group_by(features.plot) %>% summarize(n_clust=n(), num=sum(pct.exp >10), pvuprop=num/n_clust)
...
data.df <- rownames_to_column(data.df, var="gene")
head(data.df)
pct <- data.df %>% group_by(id) %>% summarize(n_cells=n(), num=sum(pct.exp > 10), mvuprop=num/n_cells)
pct
write.csv(pct, file=paste0(output,"MvU_sDEGspercluster.csv"))
dist <- data.df %>% group_by(features.plot) %>% summarize(n_clust=n(), num=sum(pct.exp >10), mvuprop=num/n_clust)
...
Bigplot <- DotPlot(mySeurat, features=AllsDegs.filtered5
...
data.df <- rownames_to_column(data.df, var="gene")
head(data.df)
dist <- data.df %>% group_by(features.plot) %>% summarize(n_clust=n(), num=sum(pct.exp >10), totalprop=num/n_clust)
head(dist)
...
uniqueclustassign
write.csv(uniqueclustassign, file=paste0(output,"VMH_clusterspecDEGs_15pct.csv"))
