...
output <- "Seurat/BNST_IndependentAnalysis/BNST_Fullmerge_Test_prepaperreclusterRound4_sexclude_malat1exclude_malat1regress_filtered4_res1.2"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
mySeurat <- readRDS("Seurat/BNST_IndependentAnalysis/BNST_Fullmerge_Test_prepaperreclusterRound4_sexclude_malat1exclude_malat1regress_nodrd3filtered3.rds")
#DefaultAssay(mySeurat) <- "integrated"
mySeurat <- subset(mySeurat, idents=c("29"), invert=TRUE)
...
mySeurat.markers <- FindAllMarkers(mySeurat, only.pos=TRUE, min.pct=0.25, min.diff.pct=0.25, logfc.threshold = 0.223, test.use="MAST")
