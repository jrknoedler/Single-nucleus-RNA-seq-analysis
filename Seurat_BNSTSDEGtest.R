#!/usr/bin/env Rscript
output <- "Seurat/BNST_IndependentAnalysis/Intact_SDEG_Plottest"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
controls <- read.table("Genelists/BNST_MvP_Unchanged.txt")
unlist(controls)
controls <- as.matrix(controls)
filtered.controls <- intersect(controls, genes.10x)
...
dev.off()
pdf(paste0(output,"_UMAP_Tac1.pdf"))
FeaturePlot(mySeurat, reduction="umap", features="Tac1", cols = c("light blue", "red"))
dev.off()
pdf(paste0(output,"_Vln_Cyp19a1"))
VlnPlot(mySeurat, features="Cyp19a1", pt.size=0)
dev.off()
pdf(paste0(output,"_Vln_Tac1"))
VlnPlot(mySeurat, features="Tac1", pt.size=0)
dev.off()
