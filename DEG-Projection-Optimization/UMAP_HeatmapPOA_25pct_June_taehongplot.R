...
pdata2 <- p$data
p2 <- ggplot(pdata2, aes(x=UMAP_1, y=UMAP_2, color=MFuRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("dark green","gold","blue"), values=rescale(c(0,1,3)), guide="colorbar", limits=c(0,3)) + theme(legend.title=element_blank())

p <- FeaturePlot(mySeurat, features=c("FrFuRatio"))
pdata3 <- p$data
p3 <- ggplot(pdata3, aes(x=UMAP_1, y=UMAP_2, color=FrFuRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("dark green","gold","deeppink1"),values=rescale(c(0,1,1.7)), guide="colorbar", limits=c(0,1.7)) + theme(legend.title=element_blank())

pdf(file=paste0(output,"Taehongcompiled.pdf"), width=30, height=10)
p1 + p2 + p3
dev.off()

pdf(file=paste0(output, "NewCombinedplot.pdf"), width=28)
