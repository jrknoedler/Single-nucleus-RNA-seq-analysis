...
output <- "X:/Seurat/MeA_IndependentAnalysis/MeA_Merged_DEGratios_JuneFinal_10pct_ratiozscaled_vert_altCCA"
...
pgdat <- p$data
pg <- ggplot(pgdat, aes(x=UMAP_1, y=UMAP_2, color=AllDEGs)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("white","orangered3"),   guide="colorbar", limits=c(0.2,0.8)) + theme(legend.position="none")
...
(pd /p2f/p2f/pg/  p1 / p2 / p3) & theme(panel.background=element_rect(fill="black"))
dev.off()