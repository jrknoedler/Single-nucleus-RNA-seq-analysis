...
library(patchwork)
library(scales)
output <- "Seurat/MeA_IndependentAnalysis/MeA_Merged_DEGratios_JuneFinal_10pct_ratiozscaled"
...
head(data.df)
pct1 <- data.df %>% group_by(id) %>% summarize(MvFr=sum(pct.exp > 5), mvfrprop=MvFr/n())
head(pct1)
...
data.df <- rownames_to_column(data.df, var="gene")
head(data.df)
pct2 <- data.df %>% group_by(id) %>% summarize(FrvM=sum(pct.exp > 5), frmprop=FrvM/n())
head(pct2)
...
pdf(file=paste0(output,"_taehongtestauto.pdf"))
...
pdata1 <- p$data
p1 <- ggplot(pdata1, aes(x=UMAP_1, y=UMAP_2, color=MFrRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("deeppink1","gold","cyan","blue"),   guide="colorbar", limits=c(-4,4)) + theme(legend.title=element_blank(),legend.key.size=unit(1, "cm")) 
...
pdata2 <- p$data
p2 <- ggplot(pdata2, aes(x=UMAP_1, y=UMAP_2, color=MFuRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("deeppink1","gold","cyan","blue"),   guide="colorbar", limits=c(-4,4)) + theme(legend.title=element_blank(),  legend.key.size=unit(1, "cm"))

p <- FeaturePlot(mySeurat, features=c("FrFuRatio"))
pdata3 <- p$data
p3 <- ggplot(pdata3, aes(x=UMAP_1, y=UMAP_2, color=FrFuRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("deeppink1","gold","cyan","blue"),  guide="colorbar", limits=c(-4,4)) + theme(legend.title=element_blank(),legend.key.size=unit(1, "cm"))
