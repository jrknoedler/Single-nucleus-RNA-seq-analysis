...
library(patchwork)
library(scales)
output <- "Seurat/VMH_IndependentAnalysis/VMH_Merged_JuneFinal_10pct_bluescalenaive"
...
pdata1 <- p$data
p1 <- ggplot(pdata1, aes(x=UMAP_1, y=UMAP_2, color=MFrRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("deeppink1","gold","cyan","blue"), 
  guide="colorbar") + theme(legend.title=element_blank(),legend.key.size=unit(1, "cm")) 
