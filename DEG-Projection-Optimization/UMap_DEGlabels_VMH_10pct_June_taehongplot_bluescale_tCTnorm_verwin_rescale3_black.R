...
library(patchwork)
library(scales)
output <- "X:/Seurat/VMH_IndependentAnalysis/VMH_Merged_JuneFinal_10pct_bluescalestack_scalebreak_condplot"
memory.limit(64000)
...
genes.10x <- (x=rownames(x=mySeurat))
unlist(genes.10x)
Dimorphic <- read.table("X:/Genelists/VMH_MvP_1.5.txt")
eDEG <- read.table("X:/Genelists/VMH_PvU_1.5.txt")
sDEG2 <- read.table("X:/Genelists/VMH_MvU_1.5.txt")
unlist(eDEG)
unlist(sDEG2)
unlist(Dimorphic)
...
AllsDegs <- read.table("X:/topGO/Total_ByRegion/VMH_Genesonly.txt")
...
p <- DimPlot(mySeurat, reduction="umap", label=TRUE, label.size=5, label.color="white")
...
pgdat <- p$data
pg <- ggplot(pgdat, aes(x=UMAP_1, y=UMAP_2, color=AllDEGs)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("white","orangered3"),   guide="colorbar", limits=c(0.2,0.8)) + theme(legend.title=element_blank(), legend.text=element_blank(),legend.key.size=unit(0.75, "cm"))
...
pc <- DimPlot(mySeurat, reduction="umap", label=FALSE, cols=c("blue", "deeppink1","green"), group.by="Hormone", shuffle=TRUE)
pce <- pc + theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.title=element_blank(), axis.text=element_blank())+theme(legend.position="none")
...
p2 <-   ggplot(pdata2, aes(x=UMAP_1, y=UMAP_2, color=MFuRatio)) +geom_point(size=0.3)+ cowplot::theme_cowplot()+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradient2(low="deeppink1", high="cyan3", mid="light gray", na.value="orange", guide="colorbar", limits=c(-2,2), breaks=c(0))  + geom_point(data=pprimedat, aes(x=UMAP_1,y=UMAP_2), size=0.3, color="blue")+ theme(axis.line=element_blank(), axis.text=element_blank())+theme(axis.title=element_blank(), axis.ticks=element_blank()) +  theme(legend.title=element_blank(), legend.text=element_blank(),  legend.key.size=unit(0.7, "cm"))

p <- FeaturePlot(mySeurat, features=c("FrFuRatio"))
pdata3 <- p$data
p3 <- ggplot(pdata3, aes(x=UMAP_1, y=UMAP_2, color=FrFuRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradient2(low="deeppink1", high="cyan3", mid="light gray", na.value="blue", guide="colorbar", limits=c(-2,2), breaks=c(0))+ theme(legend.title=element_blank(), legend.text=element_blank(),  legend.key.size=unit(0.7, "cm"))

pdf(file=paste0(output,"Taehongcompiledbluescale.pdf"), width=10, height=40)
p1 + p2 + p3
dev.off()

win.metafile(file="VMH_Stackedplots_metafile.wmf", height=24, width=5)
(pd /p2f/pc/pg/  p1 / p2 / p3) & theme(panel.background=element_rect(fill="black"))
dev.off()
win.metafile(file="VMH_celltype.wmf")
pd & theme(panel.background=element_rect(fill="black"))
dev.off()
win.metafile(file="VMH_nt.wmf")
p2f & theme(panel.background=element_rect(fill="black"))
dev.off()
win.metafile(file="VMH_condition.wmf")
pc& theme(panel.background=element_rect(fill="black"))
dev.off()
win.metafile(file="VMH_DEGs.wmf")
pg & theme(panel.background=element_rect(fill="black"))
dev.off()
win.metafile(file="VMH_Mvfr.wmf")
p1
dev.off()
win.metafile(file="VMH_MvFnr.wmf")
p2
dev.off()
win.metafile(file="VMH_FrvFnr.wmf")
p3
dev.off()