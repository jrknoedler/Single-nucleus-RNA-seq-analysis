...
library(scales)
library(patchwork)
output <- "X:/Seurat/POA_IndependentAnalysis/POA_Merged_tCTDEGs_JuneFinal_10pct_zscaledratiovertbreakscale"
memory.limit(size=64000)
mySeurat <- readRDS("X:/Seurat/POA_IndependentAnalysis/POA_Fullmerge_Test_prepaperreclusterRound2_sexclude_malat1regress_filtered6_redo.rds")
...
unlist(genes.10x)
Dimorphic <- read.table("X:/Genelists/POA_MvP_1.5cutoff.txt")
eDEG <- read.table("X:/Genelists/POA_PvU_1.5cutoff.txt")
sDEG2 <- read.table("X:/Genelists/POA_MvU_1.5cutoff.txt")
unlist(eDEG)
unlist(sDEG2)
unlist(Dimorphic)
MFrNUM <- nrow(Dimorphic)
MFuNUM <- nrow(sDEG2)
FrFuNUM <- nrow(eDEG)
...
AllsDegs <- read.table("X:/topGO/Total_ByRegion/POA_Genesonly.txt")
...
mySeuratx <- subset(mySeurat, idents=c("10","30","36"), invert=TRUE)
mySeuraty <- subset(mySeurat, idents=c("30","36"))
mySeuratz <- subset(mySeurat, idents=c("10"))
...
pdata1 <- p$data
p <- DimPlot(mySeuraty, reduction="umap")
palpha <- p$data
p <- DimPlot(mySeuratz, reduction="umap")
pbeta <- p$data
p1 <- ggplot(pdata1, aes(x=UMAP_1, y=UMAP_2, color=MFrRatio)) + cowplot::theme_cowplot()+geom_point(size=0.03)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradient2(low="deeppink1", high="cyan3", mid="light gray", na.value="purple", guide="colorbar", limits=c(-2,2), breaks=c(0)) + theme(legend.position="none")+  geom_point(data=palpha, aes(x=UMAP_1,y=UMAP_2), size=0.3, color="blue")+cowplot::theme_cowplot() + theme(axis.line=element_blank(), axis.text=element_blank())+theme(axis.title=element_blank(), axis.ticks=element_blank())+ geom_point(data=pbeta, aes(x=UMAP_1,y=UMAP_2), size=0.3, color="purple")+cowplot::theme_cowplot() + theme(axis.line=element_blank(), axis.text=element_blank())+theme(axis.title=element_blank(), axis.ticks=element_blank())+ theme(legend.position="none")
...
pdf(file=paste0(output, "NewCombinedplotbluescalevert_black.pdf"), height=21, width=3.5)
(pd /p2f/pg/  p1 / p2 / p3))
dev.off()