...
p1 <- ggplot(pdata1, aes(x=UMAP_1, y=UMAP_2, color=MFrRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradient2(low="deeppink1", high="cyan3", mid="light gray", na.value="royalblue3", guide="colorbar", limits=c(-2,2), breaks=c(0))  + theme(legend.position="none") + geom_point(data=pprimedat, aes(x=UMAP_1,y=UMAP_2), size=0.3, color="light gray")+cowplot::theme_cowplot() + theme(axis.line=element_blank(), axis.text=element_blank()) +theme(axis.title=element_blank(), axis.ticks=element_blank())  +theme(legend.position="none")
