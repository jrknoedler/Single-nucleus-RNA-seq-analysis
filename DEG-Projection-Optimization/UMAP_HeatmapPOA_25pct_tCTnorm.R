...
library(scales)
library(patchwork)
output <- "Seurat/POA_IndependentAnalysis/POA_Merged_tCTDEGs_JuneFinal_25pct_naivetCTnorm"
...
data.df <- rownames_to_column(data.df, var="gene")
head(data.df)
pct3 <- data.df %>% group_by(id) %>% summarize(MfrTot=sum(pct.exp > 25))
...
head(comb)
MFrrat <- comb %>% summarize(MFrNorm=(MvFr-FrvM)/MFrNUM)
...
data.df <- rownames_to_column(data.df, var="gene")
head(data.df)
pct4 <- data.df %>% group_by(id) %>% summarize(MvFr=sum(pct.exp > 25), mvfrprop=MvFr/n())
head(pct3)
...
data.df <- rownames_to_column(data.df, var="gene")
head(data.df)
pct5 <- data.df %>% group_by(id) %>% summarize(FrvM=sum(pct.exp > 25), frmprop=FrvM/n())
...
data.df <- rownames_to_column(data.df, var="gene")
head(data.df)
pct6 <- data.df %>% group_by(id) %>% summarize(MfrTot=sum(pct.exp > 25))
...
comb2
MFurat <- comb2 %>% summarize(MFuNorm=(MvFr-FrvM)/MFuNUM)
MFurat
...
data.df <- rownames_to_column(data.df, var="gene")
head(data.df)
pct7 <- data.df %>% group_by(id) %>% summarize(MvFr=sum(pct.exp > 25), mvfrprop=MvFr/n())
head(pct5)
...
data.df <- rownames_to_column(data.df, var="gene")
head(data.df)
pct8 <- data.df %>% group_by(id) %>% summarize(FrvM=sum(pct.exp > 25), frmprop=FrvM/n())
head(pct8)
...
data.df <- rownames_to_column(data.df, var="gene")
head(data.df)
pct9 <- data.df %>% group_by(id) %>% summarize(MfrTot=sum(pct.exp > 25))
...
head(comb)
FrFurat <- comb %>% summarize(FrFuNorm=(MvFr-FrvM)/FrFuNUM)
