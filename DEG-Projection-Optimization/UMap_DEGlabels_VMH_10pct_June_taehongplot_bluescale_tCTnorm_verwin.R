...
library(patchwork)
library(scales)
output <- "X:/Seurat/VMH_IndependentAnalysis/VMH_Merged_JuneFinal_10pct_bluescalestack"
...
p <- DimPlot(mySeurat, reduction="umap", label=TRUE, label.size=4)
...
pdata1 <- p$data
p1 <- ggplot(pdata1, aes(x=UMAP_1, y=UMAP_2, color=MFrRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("deeppink1","cyan2"), guide="colorbar",   limits=c(-4,4), breaks=c(0)) + theme(legend.title=element_blank(), legend.text=element_blank(), legend.key.size=unit(0.75, "cm"))
...
pdata2 <- p$data
p2 <- ggplot(pdata2, aes(x=UMAP_1, y=UMAP_2, color=MFuRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("deeppink1","cyan2"), guide="colorbar", limits=c(-4,4), breaks=c(0)) + theme(legend.title=element_blank(), legend.text=element_blank(), legend.key.size=unit(0.75, "cm")) 

p <- FeaturePlot(mySeurat, features=c("FrFuRatio"))
pdata3 <- p$data
p3 <- ggplot(pdata3, aes(x=UMAP_1, y=UMAP_2, color=FrFuRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("deeppink1","cyan2"), guide="colorbar", limits=c(-4,4), breaks=c(0))+ theme(legend.title=element_blank(), legend.text=element_blank(),  legend.key.size=unit(0.75, "cm"))

pdf(file=paste0(output,"Taehongcompiledbluescale.pdf"), width=10, height=40)
p1 + p2 + p3
dev.off()

pdf(file=paste0(output, "NewCombinedplotbluescalevert.pdf"), height=17.5, width=4)
(pd /pg/  p1 / p2 / p3) 
dev.off()