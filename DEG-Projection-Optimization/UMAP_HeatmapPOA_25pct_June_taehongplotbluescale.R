...
library(scales)
library(patchwork)
output <- "Seurat/POA_IndependentAnalysis/POA_Merged_tCTDEGs_JuneFinal_25pct_"
...
pdata1 <- p$data
p1 <- ggplot(pdata1, aes(x=UMAP_1, y=UMAP_2, color=MFrRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("deeppink1","gold","cyan","blue"), values= rescale(c(0.2,1,3,6)),guide="colorbar", limits=c(0.2,6), breaks=c(1)) + theme(legend.title=element_blank(), legend.text=element_blank(), legend.key.size=unit(1, "cm"))
...
pdata2 <- p$data
p2 <- ggplot(pdata2, aes(x=UMAP_1, y=UMAP_2, color=MFuRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("deeppink1","gold","cyan","blue"), values=rescale(c(0.2,1,2,3)), guide="colorbar", limits=c(0.2,3), breaks=c(1)) + theme(legend.title=element_blank(), legend.text=element_blank(), legend.key.size=unit(1, "cm"))

p <- FeaturePlot(mySeurat, features=c("FrFuRatio"))
pdata3 <- p$data
p3 <- ggplot(pdata3, aes(x=UMAP_1, y=UMAP_2, color=FrFuRatio)) + cowplot::theme_cowplot()+geom_point(size=0.3)+theme(axis.line=element_blank(), axis.ticks=element_blank(), axis.text=element_blank(), axis.title=element_blank()) + scale_colour_gradientn(colors=c("deeppink1","gold","cyan","blue"), values=rescale(c(0.2,1,1.35,1.7)), guide="colorbar", limits=c(0.2,1.7), breaks=c(1)) + theme(legend.title=element_blank(), legend.text=element_blank(), legend.key.size=unit(1, "cm"))

pdf(file=paste0(output,"Taehongcompiledbluescale.pdf"), width=30, height=10)
p1 + p2 + p3
dev.off()

pdf(file=paste0(output, "NewCombinedplotbluesacle.pdf"), width=28)
