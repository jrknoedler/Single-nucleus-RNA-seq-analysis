...
output <- "Seurat/BNST_MalevUnprimed_Exploratory/BNST_MalevUnprimed_doubletsremoved_filtered2_res1"
...
mySeurat
filteredSeurat <- subset(mySeurat, idents=c("41"), invert=TRUE)
DefaultAssay(filteredSeurat) <- "integrated"
filteredSeurat <- RunPCA(filteredSeurat, verbose=TRUE)
filteredSeurat <- FindNeighbors(filteredSeurat, dims=1:25, verbose=TRUE)
filteredSeurat <- FindClusters(filteredSeurat, verbose=TRUE, resolution=1.0)
