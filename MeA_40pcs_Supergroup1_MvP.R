...
Primed$sex <- "Primed"
Male$sex <- "Male" 
Primed <- subset(Primed, idents=c("15","21","7","14"))
Male <- subset(Male, idents=c("18","12","14","10"))
...
markers <- FindMarkers(mySeurat, ident.1="Primed", ident.2="Male", min.pct=0, logfc.threshold=0, test.use="DESeq2")
write.csv(markers, file="Seurat/MeA_IndependentAnalysis/MeA_Supergroup1_res1_40pcs_MvP_SDEGsDESeq2.csv")