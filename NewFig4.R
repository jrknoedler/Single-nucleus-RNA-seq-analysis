...
VMHdots <- VMHdata %>% 
  filter(pct.exp > 25) %>% ggplot(aes(x=features.plot, y=factor(id, levels=VMHlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_nothing() +
  geom_point() + 
...
MeAdots <- MeAdata %>% 
  filter(pct.exp > 25) %>% ggplot(aes(x=features.plot, y=factor(id, levels=MeAlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_nothing() +
  geom_point() + 
  scale_colour_gradientn(colours = c("blue","red"), limits=c(-1.5,2.5))+ scale_size(limits=c(25,100)) +
  theme(axis.line  = element_blank())   + theme(axis.title.x=element_blank()) +
  theme(axis.text.x = element_blank(), axis.text.y=element_text(hjust=1)) +
...
meaout$CPM <- "CPM"
#typemarkers <- typemarkers[,data$features.plot[["order"]]]
meacpm <- ggplot(meaout, aes(x=factor(typemarkers2, levels=typemarkers2), y=CPM, color=log10(max))) +cowplot::theme_nothing() + geom_point(size=5) + scale_color_viridis(limits=c(2.2,4.2)) +  theme(axis.line  = element_blank())   + theme(axis.title=element_blank()) + labs(x="") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + theme(axis.ticks = element_blank())
...
POAdots <- POAdata %>% 
  filter(pct.exp > 25) %>% ggplot(aes(x=features.plot, y=factor(id, levels=POAlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_nothing() +
  geom_point() + 
  scale_colour_gradientn(colours = c("blue","red"), limits=c(-1.5,2.5))+ scale_size(limits=c(25,100)) +
  theme(axis.line  = element_blank())   + theme(axis.title.x=element_blank()) +
  theme(axis.text.x = element_blank(), axis.text.y=element_text(hjust=1)) +
...
POAout$CPM <- "CPM"
#typemarkers <- typemarkers[,data$features.plot[["order"]]]
poacpm <- ggplot(POAout, aes(x=factor(POAtypemarkers2, levels=POAtypemarkers2), y=CPM, color=log10(max))) +cowplot::theme_nothing() + geom_point(size=5) + scale_color_viridis(limits=c(2.2,4.2)) +  theme(axis.line  = element_blank())   + theme(axis.title=element_blank()) + labs(x="") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + theme(axis.ticks = element_blank())
...
BNSTdots <- BNSTdata %>% 
  filter(pct.exp > 25) %>% ggplot(aes(x=features.plot, y=factor(id, levels=BNSTlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_nothing() +
  geom_point() + 
  scale_colour_gradientn(colours = c("blue","red"), limits=c(-1.5,2.5))+ scale_size(limits=c(25,100)) +
  theme(axis.line  = element_blank())   + theme(axis.title.x=element_blank()) +
  theme(axis.text.x = element_blank(), axis.text.y=element_text(hjust=1)) +
...
BNSTout$CPM <- "CPM"
#typemarkers <- typemarkers[,data$features.plot[["order"]]]
BNSTcpm <- ggplot(BNSTout, aes(x=factor(BNSTtypemarkers2, levels=BNSTtypemarkers2), y=CPM, color=log10(max))) +cowplot::theme_nothing() + geom_point(size=5) + scale_color_viridis(limits=c(2.2,4.2)) +  theme(axis.line  = element_blank())   + theme(axis.title=element_blank()) + labs(x="") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + theme(axis.ticks = element_blank())


BNSTplot <- (BNSTdots/BNSTcpm) + plot_layout(heights=c("1","0.05"))

pdf(file="Seurat/Fullfig4.pdf", width=74, height=8)
