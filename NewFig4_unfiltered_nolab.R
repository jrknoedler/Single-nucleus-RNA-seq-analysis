...
VMHdots <- VMHdata %>% ggplot(aes(x=features.plot, y=factor(id, levels=VMHlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_nothing() +
  geom_point() + 
  scale_colour_gradientn(colours = c("yellow","red2"), na.value="yellow", limits=c(-1.5,2.5))+ scale_size(limits=c(1,100)) +
...
vmhcpm <- ggplot(vmhout, aes(x=factor(vmhtypemarkers, levels=vmhtypemarkers), y=CPM, color=log10(max))) +cowplot::theme_nothing() + geom_point(size=5) + scale_color_viridis(limits=c(1,4.5)) +  theme(axis.line  = element_blank())   + theme(axis.title=element_blank()) + labs(x="") + theme(axis.text.x = element_blank()) + theme(axis.ticks = element_blank())
...
MeAdots <- MeAdata %>% ggplot(aes(x=features.plot, y=factor(id, levels=MeAlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_nothing() +
  geom_point() + 
  scale_colour_gradientn(colours = c("yellow","red2"), na.value="yellow", limits=c(-1.5,2.5))+ scale_size(limits=c(1,100)) +
  theme(axis.line  = element_blank())   + theme(axis.title.x=element_blank()) +
  theme(axis.text.x = element_blank(), axis.text.y=element_blank()) +
...
meaout$CPM <- "CPM"
#typemarkers <- typemarkers[,data$features.plot[["order"]]]
meacpm <- ggplot(meaout, aes(x=factor(typemarkers, levels=typemarkers), y=CPM, color=log10(max))) +cowplot::theme_nothing() + geom_point(size=5) + scale_color_viridis(limits=c(1,4.5)) +  theme(axis.line  = element_blank())   + theme(axis.title=element_blank()) + labs(x="") + theme(axis.text.x = element_blank()) + theme(axis.ticks = element_blank())
...
POAdots <- POAdata  %>% ggplot(aes(x=features.plot, y=factor(id, levels=POAlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_nothing() +
  geom_point() + 
  scale_colour_gradientn(colours = c("yellow","red2"), na.value="yellow", limits=c(-1.5,2.5))+ scale_size(limits=c(1,100)) +
  theme(axis.line  = element_blank())   + theme(axis.title.x=element_blank()) +
  theme(axis.text.x = element_blank(), axis.text.y=element_blank()) +
...
POAout$CPM <- "CPM"
#typemarkers <- typemarkers[,data$features.plot[["order"]]]
poacpm <- ggplot(POAout, aes(x=factor(POAtypemarkers, levels=POAtypemarkers), y=CPM, color=log10(max))) +cowplot::theme_nothing() + geom_point(size=5) + scale_color_viridis(limits=c(1,4.5)) +  theme(axis.line  = element_blank())   + theme(axis.title=element_blank()) + labs(x="") + theme(axis.text.x = element_blank()) + theme(axis.ticks = element_blank())
...
BNSTdots <- BNSTdata  %>% ggplot(aes(x=features.plot, y=factor(id, levels=BNSTlevels), color=avg.exp.scaled, size=pct.exp)) +  cowplot::theme_nothing() +
  geom_point() + 
  scale_colour_gradientn(colours = c("yellow","red2"),na.value="yellow", limits=c(-1.5,2.5))+ scale_size(limits=c(1,100)) +
  theme(axis.line  = element_blank())   + theme(axis.title.x=element_blank()) +
  theme(axis.text.x = element_blank(), axis.text.y=element_blank()) +
...
BNSTout$CPM <- "CPM"
#typemarkers <- typemarkers[,data$features.plot[["order"]]]
BNSTcpm <- ggplot(BNSTout, aes(x=factor(BNSTtypemarkers, levels=BNSTtypemarkers), y=CPM, color=log10(max))) +cowplot::theme_nothing() + geom_point(size=5) + scale_color_viridis(limits=c(1,4.5)) +  theme(axis.line  = element_blank())   + theme(axis.title=element_blank()) + labs(x="") + theme(axis.text.x = element_blank()) + theme(axis.ticks = element_blank())


BNSTplot <- (BNSTdots/BNSTcpm) + plot_layout(heights=c("1","0.05"))

pdf(file="Seurat/Fullfig4_nolabelsunfiltered.pdf", width=82, height=8)
(BNSTplot|MeAplot|POAplot|VMHplot)+plot_layout(widths=c("1","0.95","1.12","0.8"))
dev.off()
