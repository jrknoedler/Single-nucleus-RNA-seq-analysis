...
output <- "Seurat/BNST_IndependentAnalysis/BNST_Fullmerge_Test_SCTFiltered1_20pcs_0.8_sanitycheck"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
library(dplyr)
mySeurat <- readRDS("Seurat/BNST_IndependentAnalysis/BNST_Fullmerge_Test_SCT20pcsres0.8sanitycheck.rds")
...
dev.off()
pdf(file=paste0(output,"Cyp19a1_Vlnsplit.pdf"), width=40)
VlnPlot(mySeurat, features=c("Cyp19a1"), pt.size=0, split.by="Hormone")
dev.off()
pdf(file=paste0(output,"Marker_Vln.pdf"), width=40, height=15)
VlnPlot(mySeurat, features=c("Gad1","Slc17a7","Esr1","Cyp19a1","Tac1"), pt.size=0, ncol=1)
dev.off()
