...
UnprimedPOA <- <- CreateSeuratObject(counts = UnprimedPOA, project = sampleID, min.cells=3, min.features=200)
MalePOA <- Read10X(data.dir = "MalePOAIntact/outs/filtered_feature_bc_matrix")
MalePOA <- <- CreateSeuratObject(counts = MalePOA, project = sampleID, min.cells=3, min.features=200)
MalePOA$sex <- "Male"
