...
tot <- cuf+guides(colour = guide_colorbar(ticks=FALSE)) + new_scale_color() +geom_point(data=Ucdat_1, aes(x=UMAP_1, y=UMAP_2, color=Trim36), size=0.3) + geom_point(data=Ucdat_2, aes(x=UMAP_1, y=UMAP_2, color=Trim36),size=0.3) + scale_color_gradientn(colors=c("gray","green"), limits=c(0, 1.6))  + theme(axis.line=element_blank(), axis.text=element_blank()) +theme(axis.title=element_blank(), axis.ticks=element_blank()) +guides(colour = guide_colorbar(ticks=FALSE))  + theme(legend.title=element_blank(), legend.text=element_blank()) 
...
vve2 <- ggplot(vat, aes(x=Tmem215, y=factor(ident, levels=rev(myp)), fill=ident)) + geom_violin(trim=TRUE, scale="width") +scale_fill_manual(values=c(rep("dark gray", 5), rep("dark gray",1), rep("dark gray",13), rep("deeppink1",1), rep("dark gray",7)))  + theme_classic() +theme(axis.title.x=element_blank(),axis.text.x=element_blank(), axis.text.y=element_blank(), axis.title.y=element_blank()) +theme(axis.line=element_line(color="black", size=1), axis.line.x=element_blank()) + scale_x_continuous(breaks=seq(0,4,1))+theme(legend.position="none")
...
vvm1 <- ggplot(vat, aes(x=Mpped1, y=factor(ident, levels=rev(mylevels)), fill=ident)) + geom_violin(trim=TRUE, scale="width") +scale_fill_manual(values=c(rep("dark gray", 5), rep("dark gray",1), rep("dark gray",13), rep("deeppink1",1), rep("dark gray",4), rep("green", 1), rep("dark gray",2)))  + theme_classic() +theme(axis.title.x=element_blank(),axis.text.x=element_blank(),axis.ticks=element_blank(), axis.line.x=element_blank(), axis.text.y=element_blank(), axis.title.y=element_blank()) +theme(axis.line=element_line(color="black", size=1)) + scale_x_continuous(breaks=seq(0,4,1))+theme(legend.position="none")
...
pdf(file=paste0(output,"S7_fmarkersrow.pdf"), width=30, height=30)
