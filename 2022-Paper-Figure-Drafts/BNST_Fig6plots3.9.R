...
dev.off()
pdf(file=paste0(output,"Neuropeptides.pdf"), width=40,height=20)
VlnPlot(mySeurat, features=c("Cyp19a1","Tac1","Tac2","Avp","Cck","Sst","Cartpt"), pt.size=0, ncol=1)
dev.off()
pdf(file=paste0(output,"Markers.pdf"), width=40,height=20)
VlnPlot(mySeurat, features=c("Esr1","Cyp19a1","Gad1","Tac1"), pt.size=0, ncol=1)
dev.off()

write.csv(Avg, file="/scratch/users/knoedler/Heatmaps/Seurat/BNST/MvPsDEGclusteravgs.csv")
pdf(file=paste0(output,"Cyp19a1vlntest_maleonlyclusters.pdf"), width=24)
VlnPlot(mySeurat, features=c("Cyp19a1"), pt.size=0, split.by="Hormone", idents=c("0","2","5","26","28"), cols=c("light blue","pink", "light green"))
dev.off()
pdf(file=paste0(output,"Cyp19a1vlntest_maleonlyclusters_shared.pdf"), height=16)
VlnPlot(mySeurat, features=c("Isl1","Syne2","Epsti1","Adamts6","Arhgap31","Cyp19a1"), ncol=1, pt.size=0, split.by="Hormone", idents=c("0","2","5","26","28"), cols=c("light blue","pink", "light green"))
dev.off()
pdf(file=paste0(output,"Cyp19a1vlntest_dimorphicclusters.pdf"), width=24)
VlnPlot(mySeurat, features=c("Cyp19a1"), pt.size=0, split.by="Hormone", idents=c("4","14","15","16","19","20"), cols=c("light blue","pink", "light green"))
dev.off()
pdf(file=paste0(output,"Cyp19a1vlntest_dimorphicclusters.pdf"), width=24)
VlnPlot(mySeurat, features=c("Cyp19a1"), pt.size=0, split.by="Hormone", idents=c("4","14","15","16","19","20"), cols=c("light blue","pink", "light green"))
dev.off()
pdf(file=paste0(output,"Cyp19a1vlntest_maleonly_sexdivergent.pdf"), height=20)
VlnPlot(mySeurat, features=c("Greb1","Socs2","Ptchd1"), ncol=1, pt.size=0, split.by="Hormone", idents=c("0","2","5","26","28"), cols=c("light blue","pink", "light green"))
dev.off()
pdf(file=paste0(output,"Greb1vln.pdf"), width=45)
VlnPlot(mySeurat, features=c("Greb1"), pt.size=0)
dev.off()
Aro.markers <- read.table("/scratch/users/knoedler/DotPlotMarkerLists/Aromarkersnew.txt")
Aro.markers <- unlist(Aro.markers)
pdf(file=paste0(output,"Cyp19a1_MarkerVln.pdf"), height=18)
VlnPlot(mySeurat, features=Aro.markers, idents=c("4","14","15","16","19","20"), ncol=1, pt.size=0)
dev.off()
...
ScaledSeurat <- ScaleData(mySeurat, features=rownames(mySeurat), do.center=TRUE, do.scale=TRUE)
pdf(file=paste0(output,"Tac1Dotplot.pdf"))
DotPlot(ScaledSeurat, features=c("Tac1"), dot.min=0)
dev.off()
pdf(file=paste0(output,"Tac1Dotplot10.pdf"))
DotPlot(ScaledSeurat, features=c("Tac1"), dot.min=0.1)
dev.off()
pdf(file=paste0(output,"Tac1Dotplot15.pdf"))
DotPlot(ScaledSeurat, features=c("Tac1"), dot.min=0.15)
dev.off()
...
mySeurat <- BuildClusterTree(mySeurat, dims=1:30, reorder=TRUE)
pdf(file=paste0(output, "newclusttree_tree.pdf"))
PlotClusterTree(mySeurat, reorder=TRUE)
dev.off()

pdf(file=paste0(output,"ngfmarkers.pdf"), width=20, height=20)
VlnPlot(mySeurat, features=c("St18","Moxd1","Nfix","Nfib","Nfia","Cdh24","Esr2","Isl1"), pt.size=0, ncol=1, idents=c("4","14","15","16","19","20","0","2","5","26","28"))
dev.off()
...
pdf(file=paste0(output,"Tac1clust16_Vln.pdf"))
VlnPlot(mySeurat, features=c("Tac1"), idents=c("16"), pt.size=0, split.by="Hormone", cols=c("light blue","pink", "light green"))
dev.off()
pdf(file=paste0(output,"Cckclust16_Vln.pdf"))
VlnPlot(mySeurat, features=c("Cck"), idents=c("16"), pt.size=0, split.by="Hormone", cols=c("light blue","pink", "light green"))
dev.off()
