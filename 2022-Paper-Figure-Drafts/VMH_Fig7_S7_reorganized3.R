...
cuf <- ggplot() + geom_point(data=Pcdat_1, aes(x=UMAP_1, y=UMAP_2, color=Cckar), size=0.3) + geom_point(data=Pcdat_2, aes(x=UMAP_1, y=UMAP_2, color=Cckar), size=0.3) +scale_colour_gradientn(colours = c("light gray","dark red"), limits=c(0, 1.2))+ cowplot::theme_cowplot() + theme(axis.line=element_blank(), axis.text=element_blank()) +theme(axis.title=element_blank(), axis.ticks=element_blank())  
tot <- cuf + new_scale_color() +geom_point(data=Ucdat_1, aes(x=UMAP_1, y=UMAP_2, color=Trim36), size=0.3) + geom_point(data=Ucdat_2, aes(x=UMAP_1, y=UMAP_2, color=Trim36),size=0.3) + scale_color_gradientn(colors=c("light gray","dark green"), limits=c(0, 1.6)) + cowplot::theme_cowplot() + theme(axis.line=element_blank(), axis.text=element_blank()) +theme(axis.title=element_blank(), axis.ticks=element_blank()) + ggtitle("Female")
...
Mcuf <- ggplot() + geom_point(data=Mccdat_1, aes(x=UMAP_1, y=UMAP_2, color=Cckar), size=0.3, alpha=0.5) + geom_point(data=Mccdat_2, aes(x=UMAP_1, y=UMAP_2, color=Cckar, alpha=0.5), size=0.3) +scale_colour_gradientn(colours = c("light gray","dark red"), limits=c(0, 1.2))+ cowplot::theme_cowplot() + theme(axis.line=element_blank(), axis.text=element_blank()) +theme(axis.title=element_blank(), axis.ticks=element_blank())  
Mtot <- Mcuf + new_scale_color() +geom_point(data=Mmcdat_1, aes(x=UMAP_1, y=UMAP_2, color=Trim36), size=0.3, alpha=0.5) + geom_point(data=Mmcdat_2, aes(x=UMAP_1, y=UMAP_2, color=Trim36),size=0.3, alpha=0.5) + scale_color_gradientn(colors=c("light gray","dark green"), limits=c(0, 1.6)) + cowplot::theme_cowplot() + theme(axis.line=element_blank(), axis.text=element_blank()) +theme(axis.title=element_blank(), axis.ticks=element_blank()) + ggtitle("Male")



pdf(file=paste0(output,"fstacktesttrim36.pdf"))
tot
dev.off()

pdf(file=paste0(output,"fullUMAPtrim36test.pdf"), width=12)
Mtot+tot
dev.off()
...
pdf(file=paste0(output,"M7_eDEGsrow.pdf"), width=20, height=30)
vve1|vve2|vve3|vve4|vve5|vve6|vve7
dev.off()
...
data <- as.matrix(data)
CPM <- RelativeCounts(data, scale.factor=1e6, verbose=TRUE)
