...
Uc <- FeaturePlot(Unprimed, features=c("Myo1h"))
Pcdat <- Pc$data
Ucdat <- Uc$data
...
Ucdat_1 <- Ucdat[Ucdat$Myo1h == 0,]
Ucdat_2 <- Ucdat[Ucdat$Myo1h >0,]
...
tot <- cuf + new_scale_color() +geom_point(data=Ucdat_1, aes(x=UMAP_1, y=UMAP_2, color=Myo1h), size=0.3) + geom_point(data=Ucdat_2, aes(x=UMAP_1, y=UMAP_2, color=Myo1h),size=0.3) + scale_color_gradientn(colors=c("gray","green"), limits=c(0, 1.6)) + cowplot::theme_cowplot() + theme(axis.line=element_blank(), axis.text=element_blank()) +theme(axis.title=element_blank(), axis.ticks=element_blank())

Mcc <- FeaturePlot(Male, features=c("Cckar"))
Mmc <- FeaturePlot(Male, features=c("Myo1h"))
Mccdat <- Mcc$data
Mmcdat <- Mmc$data
...
Mmcdat_1 <- Mmcdat[Mmcdat$Myo1h == 0,]
Mmcdat_2 <- Mmcdat[Mmcdat$Myo1h >0,]
...
Mtot <- Mcuf + new_scale_color() +geom_point(data=Mmcdat_1, aes(x=UMAP_1, y=UMAP_2, color=Myo1h), alpha=0.5, size=0.3) + geom_point(data=Mmcdat_2, aes(x=UMAP_1, y=UMAP_2, color=Myo1h), alpha=0.5,size=0.3) + scale_color_gradientn(colors=c("gray","green"), limits=c(0, 1.6)) + cowplot::theme_cowplot() + theme(axis.line=element_blank(), axis.text=element_blank()) +theme(axis.title=element_blank(), axis.ticks=element_blank())+theme(legend.position="none")
...
pdf(file=paste0(output,"S7_fmarkersrow.pdf"), width=23, height=30)
vvm1|vvm2|vvm4|vvm5|vvm6|vvm7
dev.off()
