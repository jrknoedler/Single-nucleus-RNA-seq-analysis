...
output <- "Seurat/BNST_IndependentAnalysis/BNST_Plotsupdate_CCAclust"
...
SFARI <- read.table("Heatmaps/BNST_SFARIgenes.txt", header=FALSE)
SFARI <- unlist(SFARI)
SFARI.filtered <- intersect(SFARI, genes.10x)

SFARI2 <- read.table("Genelists/SFARI_score1mouse.txt", header=FALSE)
SFARI2<- unlist(SFARI2)
SFARI2.filtered <- intersect(SFARI2, genes.10x)
...
mySeurat <- AddModuleScore(mySeurat, features=list(SFARI.filtered), assay="RNA", name="SFARIDEGs")
mySeurat <- AddModuleScore(mySeurat, features=list(SFARI2.filtered), assay="RNA", name="SFARIAll")
pdf(file=paste0(output,"modvln.pdf"), width=30, height=20)
VlnPlot(mySeurat, features=c("maleUP1","FrUP1","Maleupcombined1","SFARIDEGs1","SFARIAll1"), pt.size=0, ncol=1)
dev.off()
...
p <- VlnPlot(mySeurat, features=c("FrUP1"), pt.size=0, ncol=1)
vlndat <- p$data
write.csv(vlndat, file=paste0(output,"FemaleModdat.csv"))
