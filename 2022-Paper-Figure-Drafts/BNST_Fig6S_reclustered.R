...
output <- "Seurat/BNST_IndependentAnalysis/BNST_Plotsupdate_reclustered"
...
dev.off()
pdf(file=paste0(output,"modvlncondsplit.pdf"), width=60, height=20)
VlnPlot(mySeurat, features=c("maleUP1","FrUP1","Maleupcombined1"), pt.size=0, ncol=1, split.by="Hormone")
dev.off()
...
mySeurat[['modulescores']] <- CreateAssayObject(data = t(x = FetchData(object = mySeurat, vars = c("maleUP1","FrUP1","Maleupcombined1"))))
pdf(file=paste0(output,"Moduleheatmaptest.pdf"))
DoHeatmap(object = mySeurat, features = c("maleUP1","FrUP1","Maleupcombined1"), assay = 'modulescores', slot = 'data')
dev.off()
...
mods <- c("maleUP1","FrUP1","Maleupcombined1")
...
avg
avg <- scale(avg)
write.csv(avg, file="Heatmaps/Seurat/BNST/ScaledModscore.csv")
pdf(file=paste0(output,"scaledmoduleheatmap.pdf"))
...
p <- VlnPlot(mySeurat, features=c("Maleupcombined1"), pt.size=0)
vlndat <- p$data

pdf(file=paste0(output,"Maleupcustvln.pdf"), width=60)
ggplot(vlndat, aes(y=Maleupcombined1, x=factor(ident, levels=mylevels))) + geom_violin(trim=TRUE, fill="dark gray", linetype="blank", scale="width") + theme_classic() +theme(axis.title.x=element_blank(),axis.text.x=element_text(size=12), axis.text.y=element_text(size=30), axis.title.y=element_blank()) +theme(axis.line=element_line(color="black", size=1))
dev.off()
