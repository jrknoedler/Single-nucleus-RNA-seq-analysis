...
output <- "Seurat/VMH_Anderson/Anderson_NonneuronalFilt4"
...
mySeurat <- readRDS("Seurat/VMH_Anderson/Anderson_NonneuronalFilt3.rds")
mySeurat <- subset(mySeurat, idents=c("33","36","48","49"), invert=TRUE)
...
DefaultAssay(mySeurat) <- "RNA"
mySeurat <- NormalizeData(mySeurat)
mySeurat.markers <- FindAllMarkers(mySeurat, only.pos=TRUE, min.pct=0.40, logfc.threshold = 0.25, test.use="MAST")
