...
output <- "Seurat/VMH_IndependentAnalysis/Xchromdiff/MvF_Xchromdiff"
...
mySeurat <- readRDS("Seurat/VMH_IndependentAnalysis/VMHMerged_arcfinalfilter_sexclude_malat1regress_res1.rds")
...
dsSeurat <- AddMetaData(dsSeurat, metadata=sex, col.name="sex")
Idents(dsSeurat) <- "Ident"
head(dsSeurat[[]])
dsSeurat <- subset(dsSeurat, nCount_RNA > 7500)
genelist <- read.table("Genelists/Xchrom.txt", header=FALSE)
...
filtered.genelist
rbp <- read.table("Genelists/RNA_Binding_Proteins.txt")
rbp <- unlist(rbp)
filtered.rbp <- intersect(rbp, genes.10x)
filtered.rbp
DefaultAssay(mySeurat) <- "RNA"
mySeurat <- NormalizeData(mySeurat)
rbp.markers <- FindAllMarkers(mySeurat, features=filtered.rbp, only.pos=TRUE, test.use="MAST", min.pct=0.25, logfc.threshold=0.25)
write.csv(rbp.markers, file=paste0(output, "_rnabindingmarkers.csv"))
...
Male <- subset(x=dsSeurat, subset=sex=="Male")
head(Male[[]])
Male <- subset(x=Male, idents=c(i))
Female <- subset(x=dsSeurat, subset=sex=="Female")
