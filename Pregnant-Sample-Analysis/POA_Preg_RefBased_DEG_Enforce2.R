...
output <- "X:Seurat/POA_Preg_RefBased/POA_RefBased_DegEnforce_1stpass"

##Load dataset 1
Published <- readRDS("Y:shared1/POA_Published.rds")
Published@meta.data$NewIntegration <- "Published"


##Load dataset 2
Pregnant <- readRDS("X:Seurat/PregPOA_Exploratory/PregPOA_Filtered3.rds")

#saveRDS(Pregnant, file="X:Integration_Pregnancy/VMH_RefBased_DEG_Enforce/PregVMH_1stPass_Filtered_Foxa1_Car8_Filtered.rds")
...
DEGs1 <- read.csv("Y:shared1/POA_AllvFemale1.5_2.txt", header = FALSE)
...
DEGs.filtered <- intersect(DEGs1, allgenes)

##Get genes to exclude from clustering (Xist, Tsix and y-chromosome genes)
exclude <- read.table("Z:Guided_integration/ExcludeIDs.txt", header=FALSE) 
...
list <- PrepSCTIntegration(list, anchor.features = features.degs, verbose=TRUE)
list <- lapply(X = list, FUN = RunPCA, features = features.degs)

anchors <- FindIntegrationAnchors(object.list = list, reference = c(1),
                                  normalization.method = "SCT", anchor.features = features.degs,
