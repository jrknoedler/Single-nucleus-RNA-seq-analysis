...
library(patchwork)
library(scales)
output <- "X:/Seurat/PregBNST_Exploratory/BNST_PregSample_PregDEGProject"
memory.limit(64000)
mySeurat <- readRDS("X:/Seurat/PregBNST_Exploratory/PregBNST_Emptydrops_Filtered6.rds")
...
unlist(genes.10x)
PregvMale <- read.table("X:/Genelists/PregnancyDEGs/BNST_PregvMale_All1.5.txt")
PregvEst <- read.table("X:/Genelists/PregnancyDEGs/BNST_PregvFr_All1.5.txt")
PregvDiest <- read.table("X:/Genelists/PregnancyDEGs/BNST_PregvFnr_All1.5.txt")
unlist(PregvEst)
unlist(PregvMale)
unlist(PregvDiest)
...
Up_DiestvPreg <- read.table("X:Genelists/PregnancyDEGs/BNST_PregvFnr_DownPreg.txt", header=FALSE)
