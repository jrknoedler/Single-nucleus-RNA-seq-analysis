...
library(patchwork)
library(scales)
output <- "X:/Seurat/MeA_IndependentAnalysis/MeA_Merged_PregDEGProject"
memory.limit(64000)
...
genes.10x <- (x=rownames(x=mySeurat))
unlist(genes.10x)
PregvMale <- read.table("X:/Genelists/PregnancyDEGs/MeA_PregvMale_All1.5.txt")
PregvEst <- read.table("X:/Genelists/PregnancyDEGs/MeA_PregvFr_All1.5.txt")
PregvDiest <- read.table("X:/Genelists/PregnancyDEGs/MeA_PregvFnr_All1.5.txt")
unlist(PregvEst)
unlist(PregvMale)
unlist(PregvDiest)
...
Up_DiestvPreg <- read.table("X:Genelists/PregnancyDEGs/MeA_PregvFnr_DownPreg.txt", header=FALSE)
...
FeaturePlot(mySeurat, features=c("IntersectPregDownDEGs"))
dev.off()
