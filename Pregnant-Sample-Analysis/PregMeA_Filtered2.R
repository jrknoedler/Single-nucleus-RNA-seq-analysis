...
output <- "Seurat/PregMeA_Exploratory/PregMeA_Filtered2"
...
mySeurat <- readRDS("Seurat/PregMeA_Exploratory/PregMeA_Filtered1.rds")
mySeurat <- subset(mySeurat, idents=c("8","2","7","15","29","32","35"), invert=TRUE)
...
VlnPlot(mySeurat, features=c("Gad1","Slc17a6","Esr1"), ncol=1, pt.size=0)
dev.off()
SeuratScaled <- ScaleData(mySeurat)
pdf(paste0(output,"_Markerdot.pdf"), width=40, height=40)
DotPlot(SeuratScaled, features=c("Gad1","Slc17a6","Esr1","Mbp","Plp1","Cxcrl1","Inpp5d"))
dev.off()
