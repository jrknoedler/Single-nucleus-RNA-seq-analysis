output <- "Seurat/POA_IndependentAnalysis/POA_Fullmerge_Test_CCAtestallstandardscaleDEGenforced_filtered5_res1.5"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
POA.comb <- readRDS("Seurat/POA_IndependentAnalysis/POA_Fullmerge_Test_CCAtestallstandardscaleDEGenforced_filtered4CCAscaledata_DEGsonly.rds")
POA.comb <- subset(POA.comb, idents=c("40"), invert=TRUE) 
...
POA.comb <- FindClusters(POA.comb, resolution=1.5)
...
VlnPlot(POA.comb, features=c("Kiss1"), split.by="Hormone", pt.size=0)
dev.off()
