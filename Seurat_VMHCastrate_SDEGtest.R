#!/usr/bin/env Rscript
output <- "Seurat/VMH_IndependentAnalysis/CastrateSDEG_20PCS_Plottest"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
mySeurat <- readRDS("Seurat/VMH_IndependentAnalysis/VMH_IndependentFiltered1_20pcs_Castrate_filtered2.rds")
head(mySeurat[[]])
mySeurat
...
dev.off()
pdf(paste0(output,"_UMAP_Uty.pdf"))
FeaturePlot(mySeurat, reduction="umap", features="Uty", cols = c("light blue", "red"))
dev.off()
pdf(paste0(output,"_UMAP_Kdm5c.pdf"))
FeaturePlot(mySeurat, reduction="umap", features="Kdm5c", cols = c("light blue", "red"))
dev.off()
pdf(paste0(output,"_UMAP_Ddx3y.pdf"))
FeaturePlot(mySeurat, reduction="umap", features="Ddx3y", cols = c("light blue", "red"))
dev.off()
