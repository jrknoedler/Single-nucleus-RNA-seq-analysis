...
output <- "Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/PseudobulkRegressDegomeds/MvP/MvP_DEGome1.5fold_reclustered"
...
dsSeurat <- readRDS("Seurat/BNST_IndependentAnalysis/BNST_Fullmerge_Test_Striatumfiltered2_sexclude_malat1regress_30pcsres1.2FINAL.rds")
...
datam <- Male@assays[["RNA"]]@counts
datam.mat <- as.matrix(datam)

datams.mat <- SampleUMI(datam.mat, max.umi=40000, upsample=FALSE, verbose=TRUE)
Maledata <- round(datams.mat)


dataf <- Female@assays[["RNA"]]@counts
dataf.mat <- as.matrix(dataf)
datafs.mat <- SampleUMI(dataf.mat, max.umi=40000, upsample=FALSE, verbose=TRUE)
Femaledata <- round(datafs.mat)
#Maledata <- Male@assays[["RNA"]]@counts
Maledata.df <- data.frame(Maledata)
...
Femaledata.df <- data.frame(t(Femaledata))
