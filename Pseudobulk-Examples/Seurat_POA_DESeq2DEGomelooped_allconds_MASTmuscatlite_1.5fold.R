...
output <- "Seurat/POA_IndependentAnalysis/Paper_DraftAnalysis/RegressDegome/MAST_muscatelite/MvP/MvP_DEGome1.5fold_reclusteredfinal_1.5fold"
mvuoutput <- "Seurat/POA_IndependentAnalysis/Paper_DraftAnalysis/RegressDegome/MAST_muscatelite/MvU/MvU_DEGome1.5fold_reclusteredfinal_1.5fold"
pvuoutput <- "Seurat/POA_IndependentAnalysis/Paper_DraftAnalysis/RegressDegome/MAST_muscatelite/PvU/PvU_DEGome1.5fold_reclusteredfinal_1.5fold"
baseoutput <- "Seurat/POA_IndependentAnalysis/Paper_DraftAnalysis/RegressDegome/MAST_muscatelite/"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
sex.dimorphism <- FindMarkers(dsSeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, test.use="MAST",  verbose=TRUE)
write.csv(sex.dimorphism, file=paste0(output,i,"_MASTDEGOME.csv"))
sex.dimorphism <- data.frame(sex.dimorphism)
padjcounts <- sex.dimorphism[sex.dimorphism$p_val_adj < 0.05,]
fccounts <- nrow(padjcounts[padjcounts$avg_logFC < -0.407 | padjcounts$avg_logFC > 0.407,])
df <- data.frame(i,fccounts)
total_dimorphic=rbind(total_dimorphic, df)
clust <- sex.dimorphism[sex.dimorphism$p_val_adj < 0.05,]
clust2 <- clust[clust$avg_logFC < -0.407 | clust$avg_logFC > 0.407,]
clust2$gene <- row.names(clust2)
master_list = rbind(master_list, clust2)
})
}
...
MvP <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Intact", ident.2="Primed", min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, test.use="MAST",verbose=TRUE)
write.csv(MvP, file=paste0(baseoutput,"MvP_Metadegs.csv"))
MvU <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Intact", ident.2="Unprimed", min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, test.use="MAST",  verbose=TRUE)
write.csv(MvU, file=paste0(baseoutput,"MvU_Metadegs.csv"))
PvU <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Primed", ident.2="Unprimed", min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, test.use="MAST",  verbose=TRUE)
