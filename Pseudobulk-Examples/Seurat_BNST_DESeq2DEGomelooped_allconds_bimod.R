...
output <- "Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/Reclustered_DEGome/Ttest/MvP/MvP_DEGome1.5fold_reclusteredfinal"
mvuoutput <- "Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/Reclustered_DEGome/Ttest/MvU/MvU_DEGome1.5fold_reclusteredfinal"
pvuoutput <- "Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/Reclustered_DEGome/Ttest/PvU/PvU_DEGome1.5fold_reclusteredfinal"
baseoutput <- "Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/Reclustered_DEGome/Ttest/"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
sex.dimorphism <- FindMarkers(dsSeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, min.pct=0, test.use="t", only.pos=FALSE, features=filtered.MvP, verbose=TRUE)
...
sex.dimorphism <- FindMarkers(dsSeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, test.use="t",min.pct=0, only.pos=FALSE,features=filtered.MvU, verbose=TRUE)
...
ident2 <- paste0(i,"_Unprimed")
sex.dimorphism <- FindMarkers(dsSeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, min.pct=0,test.use="t", only.pos=FALSE, features=filtered.PvU, verbose=TRUE)
...
MvP <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Intact", ident.2="Primed", min.cells.group=0, logfc.threshold=0, min.pct=0, test.use="t",only.pos=FALSE, features=filtered.MvP, verbose=TRUE)
write.csv(MvP, file=paste0(baseoutput,"MvP_Metadegs.csv"))
MvU <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Intact", ident.2="Unprimed", min.cells.group=0, logfc.threshold=0, min.pct=0, test.use="t",only.pos=FALSE, features=filtered.MvU, verbose=TRUE)
write.csv(MvU, file=paste0(baseoutput,"MvU_Metadegs.csv"))
PvU <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Primed", ident.2="Unprimed", min.cells.group=0, logfc.threshold=0, min.pct=0,test.use="t", only.pos=FALSE, features=filtered.PvU, verbose=TRUE)
