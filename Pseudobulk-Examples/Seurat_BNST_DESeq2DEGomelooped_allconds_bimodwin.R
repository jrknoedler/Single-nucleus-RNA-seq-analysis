...
output <- "X:/Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/Reclustered_DEGome/Bimod/MvP/MvP_DEGome1.5fold_reclusteredfinal"
mvuoutput <- "X:/Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/Reclustered_DEGome/Bimod/MvU/MvU_DEGome1.5fold_reclusteredfinal"
pvuoutput <- "X:/Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/Reclustered_DEGome/Bimod/PvU/PvU_DEGome1.5fold_reclusteredfinal"
baseoutput <- "X:/Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/Reclustered_DEGome/Bimod/"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
genelist <- read.table("X:/Genelists/BNST_MvP_1.5cutoff.txt", header=FALSE)
...
sex.dimorphism <- FindMarkers(dsSeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, min.pct=0, test.use="bimod", only.pos=FALSE, features=filtered.MvP, verbose=TRUE)
...
genelist <- read.table("X:/Genelists/BNST_MvU_1.5cutoff.txt", header=FALSE)
genelist <- unlist(genelist)
dim(x=dsSeurat)
genes.10x <- (x=rownames(x=dsSeurat))
unlist(genes.10x)
...
sex.dimorphism <- FindMarkers(dsSeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, test.use="bimod",min.pct=0, only.pos=FALSE,features=filtered.MvU, verbose=TRUE)
...
genelist <- read.table("X:/Genelists/BNST_PvU_1.5cutoff.txt", header=FALSE)
genelist <- unlist(genelist)
dim(x=dsSeurat)
genes.10x <- (x=rownames(x=dsSeurat))
unlist(genes.10x)
...
ident2 <- paste0(i,"_Unprimed")
sex.dimorphism <- FindMarkers(dsSeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, min.pct=0,test.use="bimod", only.pos=FALSE, features=filtered.PvU, verbose=TRUE)
...
MvP <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Intact", ident.2="Primed", min.cells.group=0, logfc.threshold=0, min.pct=0, test.use="bimod",only.pos=FALSE, features=filtered.MvP, verbose=TRUE)
write.csv(MvP, file=paste0(baseoutput,"MvP_Metadegs.csv"))
MvU <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Intact", ident.2="Unprimed", min.cells.group=0, logfc.threshold=0, min.pct=0, test.use="bimod",only.pos=FALSE, features=filtered.MvU, verbose=TRUE)
write.csv(MvU, file=paste0(baseoutput,"MvU_Metadegs.csv"))
PvU <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Primed", ident.2="Unprimed", min.cells.group=0, logfc.threshold=0, min.pct=0,test.use="bimod", only.pos=FALSE, features=filtered.PvU, verbose=TRUE)
