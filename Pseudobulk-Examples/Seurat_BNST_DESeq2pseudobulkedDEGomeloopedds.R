...
output <- "Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/PseudobulkRegressDegomeds/MvP/MvP_PBDEGome1.5fold_reclustered"
...
Malemat <- Male@assays[["RNA"]]@counts
Malemat.df <- as.matrix(Malemat)
malemedian <- median(colSums(Malemat.df))
malemedian
datam <- SampleUMI(Male, max.umi=malemedian, upsample=FALSE, verbose=TRUE)
dim(datam)
datamr <- round(datam)
dim(datamr)
...
dataf <- SampleUMI(Female, max.umi=malemedian, upsample=FALSE, verbose=TRUE)
datafr <- round(dataf)
#Female <- subset(Female, subset=nCount_RNA > 7500 & nCount_RNA < 40000)
#Maledata <- Male@assays[["RNA"]]@counts
#dim(Maledata)
Maledata.df <- as.data.frame(t(datamr))
dim(Maledata.df)
...
#dim(Femaledata)
Femaledata.df <- as.data.frame(t(datafr))
dim(Femaledata.df)
