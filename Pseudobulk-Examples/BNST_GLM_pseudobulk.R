...
output <- "X:/Seurat/BNST_IndependentAnalysis/GLM_MetaDEGsproteincoding"
...
genelist <- read.table("X:/vM21_Proteincoding_Genesybols.txt", header=FALSE)
...
restargeted <- resMalevPrimed[filtered.genelist,]
...
## Order by adjusted p-value
restargeted <- resMalevUnprimed[filtered.genelist,]
restargeted$padj <- p.adjust(restargeted$pvalue, method="BH")
table(restargeted$padj<0.05)
...
## Order by adjusted p-value
restargeted <- resPrimedvUnprimed[filtered.genelist,]
restargeted$padj <- p.adjust(restargeted$pvalue, method="BH")
table(restargeted$padj<0.05)
