...
output <- "X:/Seurat/POA_IndependentAnalysis/GLM_MetaDEGs10pseudo"
...
genelist <- read.table("X:/Genelists/POA_MvP_1.5cutoff.txt", header=FALSE)
...
genelist <- read.table("X:/Genelists/POA_MvU_1.5cutoff.txt", header=FALSE)
genelist <- unlist(genelist)
dim(x=dsSeurat)
genes.10x <- (x=rownames(x=dsSeurat))
unlist(genes.10x)
...
genelist <- read.table("X:/Genelists/POA_PvU_1.5cutoff.txt", header=FALSE)
genelist <- unlist(genelist)
dim(x=dsSeurat)
genes.10x <- (x=rownames(x=dsSeurat))
unlist(genes.10x)
