...
output <- "Seurat/VMH_IndependentAnalysis/Paper_DraftAnalysis/RegressDegome/MAST/MvP/MvP_DEGome1.5fold_reclusteredfinal"
mvuoutput <- "Seurat/VMH_IndependentAnalysis/Paper_DraftAnalysis/RegressDegome/MAST/MvU/MvU_DEGome1.5fold_reclusteredfinal"
pvuoutput <- "Seurat/VMH_IndependentAnalysis/Paper_DraftAnalysis/RegressDegome/MAST/PvU/PvU_DEGome1.5fold_reclusteredfinal"
baseoutput <- "Seurat/VMH_IndependentAnalysis/Paper_DraftAnalysis/RegressDegome/MAST"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
genelist <- read.table("Genelists/VMH_MvP_1.5.txt", header=FALSE)
...
sex.dimorphism <- FindMarkers(dsSeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, test.use="MAST", features=filtered.MvP, verbose=TRUE)
...
genelist <- read.table("Genelists/VMH_MvU_1.5.txt", header=FALSE)
genelist <- unlist(genelist)
dim(x=dsSeurat)
genes.10x <- (x=rownames(x=dsSeurat))
unlist(genes.10x)
...
sex.dimorphism <- FindMarkers(dsSeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, test.use="MAST", features=filtered.MvU, verbose=TRUE)
...
ident2 <- paste0(i,"_Unprimed")
sex.dimorphism <- FindMarkers(dsSeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, test.use="MAST", features=filtered.PvU, verbose=TRUE)
...
MvP <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Intact", ident.2="Primed", min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, test.use="MAST", features=filtered.MvP, verbose=TRUE)
write.csv(MvP, file=paste0(baseoutput,"MvP_Metadegs.csv"))
MvU <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Intact", ident.2="Unprimed", min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, test.use="MAST", features=filtered.MvU, verbose=TRUE)
write.csv(MvU, file=paste0(baseoutput,"MvU_Metadegs.csv"))
PvU <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Primed", ident.2="Unprimed", min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, test.use="MAST", features=filtered.PvU, verbose=TRUE)
