...
pregvpoutput <- "Seurat/VMH_Barcode_Transfer/Cluster_DEGs/MAST_noFilter/Preg_v_Primed/PregvPrimed_DEGome_"
pregvuoutput <- "Seurat/VMH_Barcode_Transfer/Cluster_DEGs/MAST_noFilter/Preg_v_Unprimed/PregvUnprimed_DEGome_"
...
genelist <- read.table("Genelists/PregnancyDEGs/UpdatedPregDEGs/VMH_PvFr_All1.5.txt", header=FALSE)
...
genelist <- read.table("Genelists/PregnancyDEGs/UpdatedPregDEGs/VMH_PregvFnr_All1.5.txt", header=FALSE)
genelist <- unlist(genelist)
dim(x=mySeurat)
genes.10x <- (x=rownames(x=mySeurat))
unlist(genes.10x)
