...
output <- "Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/Reclustered_DEGome/MvP/MvP_DEGome1.5fold_reclusteredfinal"
mvuoutput <- "Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/Reclustered_DEGome/MvU/MvU_DEGome1.5fold_reclusteredfinal"
pvuoutput <- "Seurat/BNST_IndependentAnalysis/Paper_DraftAnalysis/Reclustered_DEGome/PvU/PvU_DEGome1.5fold_reclusteredfinal"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
restargeted <- res[filtered.genelist,]
restargeted$padj <- p.adjust(restargeted$pvalue, method="BH")
write.csv(restargeted, file=paste0(mvuoutput,i,"_sDEGpadjust.csv"))
restargeted <- na.omit(restargeted)
padjcounts <- nrow(restargeted[restargeted$padj < 0.05,])
df <- data.frame(i, padjcounts)
total_dimorphic=rbind(total_dimorphic, df)
...
restargeted <- res[filtered.genelist,]
restargeted$padj <- p.adjust(restargeted$pvalue, method="BH")
write.csv(restargeted, file=paste0(pvuoutput,i,"_sDEGpadjust.csv"))
restargeted <- na.omit(restargeted)
padjcounts <- nrow(restargeted[restargeted$padj < 0.05,])
df <- data.frame(i, padjcounts)
total_dimorphic=rbind(total_dimorphic, df)
