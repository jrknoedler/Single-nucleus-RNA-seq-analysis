...
output <- "X:/Seurat/VMH_IndependentAnalysis/VMH_MetaDEGslocal"
...
genelist <- read.table("X:/Genelists/VMH_MvP_1.5.txt", header=FALSE)
...
genelist <- read.table("X:/Genelists/VMH_MvU_1.5.txt", header=FALSE)
genelist <- unlist(genelist)
dim(x=dsSeurat)
genes.10x <- (x=rownames(x=dsSeurat))
unlist(genes.10x)
...
genelist <- read.table("X:/Genelists/VMH_PvU_1.5.txt", header=FALSE)
genelist <- unlist(genelist)
dim(x=dsSeurat)
genes.10x <- (x=rownames(x=dsSeurat))
unlist(genes.10x)
