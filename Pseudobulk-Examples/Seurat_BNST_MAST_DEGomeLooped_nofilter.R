...
pregvpoutput <- "Seurat/BNST_Barcode_Transfer/Cluster_DEGs/MAST_NoFilter/Preg_v_Primed/PregvPrimed_DEGome_"
pregvuoutput <- "Seurat/BNST_Barcode_Transfer/Cluster_DEGs/MAST_NoFilter/Preg_v_Unprimed/PregvUnprimed_DEGome_"
...
mySeurat <- readRDS("Seurat/BNST_Barcode_Transfer/BNST_Barcode_Lift_Filtered3.rds")
...
genelist <- read.table("Genelists/PregnancyDEGs/UpdatedPregDEGs/BNST_PregvFr_All1.5.txt", header=FALSE)
...
dsSeurat <- NormalizeData(mySeurat)
for (i in 1:clustno){
try({
ident1 <- paste0(i,"_Pregnant")
ident2 <- paste0(i,"_Primed")
sex.dimorphism <- FindMarkers(mySeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, features=filtered.PregvPrimed, verbose=TRUE, test.use="MAST")
...
genelist <- read.table("Genelists/PregnancyDEGs/UpdatedPregDEGs/BNST_PregvFnr_All1.5.txt", header=FALSE)
genelist <- unlist(genelist)
dim(x=mySeurat)
genes.10x <- (x=rownames(x=mySeurat))
unlist(genes.10x)
...
sex.dimorphism <- FindMarkers(mySeurat, assay="RNA", ident.1 = ident1, ident.2=ident2, min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE,features=filtered.PregvUnprimed, verbose=TRUE, test.use="MAST")
...
PregvPrimed <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Pregnant", ident.2="Primed", min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, features=filtered.PregvPrimed, verbose=TRUE)
write.csv(PregvPrimed, file=paste0(pregvpoutput,"PregvPrimed_Metadegs.csv"))
PregvUnprimed <- FindMarkers(dsSeurat, assay="RNA", ident.1 = "Pregnant", ident.2="Unprimed", min.cells.group=0, logfc.threshold=0, min.pct=0, only.pos=FALSE, features=filtered.PregvUnprimed, verbose=TRUE)
