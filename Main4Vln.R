...
BNSTtypemarkers <- read.table("DotPlotMarkerLists/BNST_ReorderedFinal_CPM.txt", header=FALSE)
BNSTtypemarkers <- unlist(BNSTtypemarkers)
BNSTtypemarkers.m <- as.matrix(BNSTtypemarkers)

BNSTtypemarkersnt <- read.table("DotPlotMarkerLists/BNST_ReorderedFinal_CPM_excitmarkers.txt", header=FALSE)
...
plotphf <- plotph & theme(axis.text.x=element_blank(), axis.text.y=element_blank(),axis.ticks=element_blank(), axis.title=element_blank(), plot.title=element_blank()) & scale_fill_manual(values=c(rep("blue",4), rep("red",32)))

pdf(file="Seurat/BNST_Phenotypes.pdf", width=50, height=20)
...
plotf <- plot & theme(axis.text.x=element_blank(), axis.text.y=element_blank(),axis.ticks=element_blank(), axis.title=element_blank(), plot.title=element_blank()) & scale_fill_manual(values=c(rep("blue",4), rep("red",32)))

pdf(file="Seurat/BNST_ViolinTest.pdf", width=60, height=163)
...
plotf2 <- plot2 & theme(axis.text.x=element_blank(), axis.text.y=element_blank(),axis.ticks=element_blank(), axis.title=element_blank(), plot.title=element_blank()) & scale_fill_manual(values=c(rep("blue",4), rep("red",32)))

pdf(file="Seurat/BNST_ViolinEdittestwide.pdf", width=95, height=38.5)
(plotf|BNSTcpm)  + plot_layout(widths=c("1","0.05")) 
dev.off()
pdf(file="Seurat/BNSTCPMwidescale.pdf", height=78)
...
vmhtypemarkers <- read.table("DotPlotMarkerLists/VMH_reorderedfinal_CPM.txt", header=FALSE)
vmhtypemarkers <- unlist(vmhtypemarkers)
vmhtypemarkers <- unlist(vmhtypemarkers)
vmhtypemarkers.m <- as.matrix(vmhtypemarkers)

vmhtypemarkersnt <- read.table("DotPlotMarkerLists/VMH_reorderedfinal_CPM_excitmarkers.txt", header=FALSE)
...
pdf(file="Seurat/VMH_ViolinTest.pdf", width=60, height=126)
VlnPlot(VMH, features=vmhtypemarkers, pt.size=0, ncol=1)
dev.off()

plot <- VlnPlot(VMH, features=rev(vmhtypemarkers), pt.size=0, ncol=1)
plotf <- plot & theme(axis.text.x=element_blank(), axis.text.y=element_blank(), axis.ticks=element_blank(), axis.title=element_blank(), plot.title=element_blank()) & scale_fill_manual(values=c(rep("blue",17), rep("red",10)))
...
pdf(file="Seurat/VMH_ViolinEdittest.pdf", width=60, height=125.6)
...
plotphf <- plotph & theme(axis.text.x=element_blank(), axis.text.y=element_blank(),axis.ticks=element_blank(), axis.title=element_blank(), plot.title=element_blank()) & scale_fill_manual(values=c(rep("blue",17), rep("red",10)))

pdf(file="Seurat/VMH_Phenotypes.pdf", width=50, height=20)
(plotphf|VMHphCPM) + plot_layout(widths=c("1","0.05")) 
dev.off()


plot2 <- VlnPlot(VMH, features=vmhtypemarkersnt, pt.size=0, ncol=1)
plotf2 <- plot2 & theme(axis.text.x=element_blank(), axis.text.y=element_blank(), axis.ticks=element_blank(), axis.title=element_blank(), plot.title=element_blank()) & scale_fill_manual(values=c(rep("blue",17), rep("red",10)))
vmhcpm <- ggplot(vmhout, aes(y=factor(vmhtypemarkers, levels=vmhtypemarkers), x=CPM, color=log10(max))) +cowplot::theme_nothing() + geom_point(size=30) + scale_color_viridis(limits=c(1,4.5)) +  theme(axis.line  = element_blank())   + theme(axis.title=element_blank()) + labs(x="") + theme(axis.text.x = element_blank()) + theme(axis.ticks = element_blank())
...
pdf(file="Seurat/VMH_ViolinEdittestwid.pdf", width=95, height=30)
(plotf|vmhcpm)  + plot_layout(widths=c("1","0.05")) 
dev.off()
pdf(file="Seurat/VMHCPMwidescale.pdf", height=78)
...
typemarkers <- read.table("DotPlotMarkerLists/MeA_ReorderedFinal_CPM.txt", header=FALSE)
...
meaout$CPM <- "CPM"
#typemarkers <- typemarkers[,data$features.plot[["order"]]]
meacpm <- ggplot(meaout, aes(y=factor(typemarkers, levels=typemarkers), x=CPM, color=log10(max))) +cowplot::theme_nothing() + geom_point(size=30) + scale_color_viridis(limits=c(1,4.5)) +  theme(axis.line  = element_blank())   + theme(axis.title=element_blank()) + labs(x="") + theme(axis.text.x = element_blank()) + theme(axis.ticks = element_blank())

plotf <- plot & theme(axis.text.x=element_blank(), axis.text.y=element_blank(), axis.ticks=element_blank(),axis.title=element_blank(), plot.title=element_blank()) & scale_fill_manual(values=c(rep("blue",19), rep("red",15)))


pdf(file="Seurat/MeA_ViolinTest.pdf", width=60, height=154.8)
(plotf|meacpm) + plot_layout(widths=c("1","0.05")) 
dev.off()

pdf(file="Seurat/MeA_ViolinTestwide.pdf", width=95, height=37)
(plotf|meacpm) + plot_layout(widths=c("1","0.05")) 
dev.off()
...
plotphf <- plotph & theme(axis.text.x=element_blank(), axis.text.y=element_blank(),axis.ticks=element_blank(), axis.title=element_blank(), plot.title=element_blank()) & scale_fill_manual(values=c(rep("blue",19), rep("red",15)))

pdf(file="Seurat/MeA_Phenotypes.pdf", width=50, height=20)
(plotphf|MeAphCPM) + plot_layout(widths=c("1","0.05")) 
dev.off()
...
POAtypemarkers <- read.table("DotPlotMarkerLists/POA_ReorderedFinal_CPM.txt", header=FALSE)
...
POAout$CPM <- "CPM"
#typemarkers <- typemarkers[,data$features.plot[["order"]]]
poacpm <- ggplot(POAout, aes(y=factor(POAtypemarkers, levels=POAtypemarkers), x=CPM, color=log10(max))) +cowplot::theme_nothing() + geom_point(size=30) + scale_color_viridis(limits=c(1,4.5)) +  theme(axis.line  = element_blank())   + theme(axis.title=element_blank()) + labs(x="") + theme(axis.text.x = element_blank()) + theme(axis.ticks = element_blank())

plotf <- plot & theme(axis.text.x=element_blank(), axis.ticks=element_blank(), axis.text.y=element_blank(), axis.title=element_blank(), plot.title=element_blank()) & scale_fill_manual(values=c(rep("blue",14), rep("red",26)))

pdf(file="Seurat/POA_ViolinTest.pdf", width=60, height=180)
(plotf|poacpm)  + plot_layout(widths=c("1","0.05")) 
dev.off()


pdf(file="Seurat/POA_ViolinTestwide.pdf", width=95, height=43)
(plotf|poacpm)  + plot_layout(widths=c("1","0.05")) 
dev.off()
...
plotphf <- plotph & theme(axis.text.x=element_blank(), axis.text.y=element_blank(),axis.ticks=element_blank(), axis.title=element_blank(), plot.title=element_blank()) & scale_fill_manual(values=c(rep("blue",14), rep("red",26)))

pdf(file="Seurat/POA_Phenotypes.pdf", width=50, height=20)
(plotphf|POAphCPM) + plot_layout(widths=c("1","0.05")) 
dev.off()
