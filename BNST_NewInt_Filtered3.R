...
output <- "Seurat/BNST_Barcode_Transfer/BNST_Barcode_Lift_Filtered3"
...
mySeurat <- readRDS("Seurat/BNST_Barcode_Transfer/BNST_Barcode_Lift_Filtered2.rds")
DefaultAssay(mySeurat) <- "SCT"
mySeurat <- subset(mySeurat, idents=c("38"), invert=TRUE)
...
mySeurat <- RunUMAP(mySeurat, reduction="pca", dims=1:30)
