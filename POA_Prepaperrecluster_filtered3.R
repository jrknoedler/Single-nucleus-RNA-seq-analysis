...
output <- "Seurat/POA_IndependentAnalysis/POA_Fullmerge_Test_prepaperreclusterRound2_sexclude_malat1regress_filtered3_30pcsres1"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
mySeurat <- readRDS("Seurat/POA_IndependentAnalysis/POA_Fullmerge_Test_prepaperreclusterRound2_sexclude_malat1regress_filtered2.rds")
#DefaultAssay(mySeurat) <- "integrated"
mySeurat <- subset(mySeurat, idents=c("46"), invert=TRUE)
...
pdf(file=paste0(output,"Dimheamap1_15.pdf"), width=40, height=40)
DimHeatmap(mySeurat, dims=1:15, cells=500, nfeatures=50, balanced=TRUE)
dev.off()
pdf(file=paste0(output,"Dimheamap15_30.pdf"), width=40, height=430)
DimHeatmap(mySeurat, dims=16:30, cells=500, nfeatures=50, balanced=TRUE)
dev.off()
...
mySeurat.markers <- FindAllMarkers(mySeurat, only.pos=TRUE, min.pct=0.25, logfc.threshold = 0.223, min.diff.pct=0.25,test.use="MAST")
