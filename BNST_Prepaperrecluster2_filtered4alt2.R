...
output <- "Seurat/BNST_IndependentAnalysis/BNST_Fullmerge_Test_prepaperreclusterRound2_sexclude_malat1exclude_malat1regress_filtered4alt2_res1.4"
library(Seurat)
library(cowplot)
library(reticulate)
library(ggplot2)
...
mySeurat <- readRDS("Seurat/BNST_IndependentAnalysis/BNST_Fullmerge_Test_prepaperreclusterRound2_sexclude_malat1exclude_malat1regress_filtered3alt2.rds")
#DefaultAssay(mySeurat) <- "integrated"
mySeurat <- subset(mySeurat, idents=c("35"), invert=TRUE)
...
mySeurat <- FindClusters(mySeurat, resolution=1.4)
