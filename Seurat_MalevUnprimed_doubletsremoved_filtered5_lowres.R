...
output <- "Seurat/POA_MalevUnprimedMerged_Exploratory/MaleUnprimed_doubletsremoved_filtered5_fewerdims"
...
mySeurat <- readRDS("Seurat/POA_MalevUnprimedMerged_Exploratory/MaleUnprimed_doubletsremoved_filtered4.rds")
mySeurat
filteredSeurat <- subset(mySeurat, idents=c("40"), invert=TRUE)
DefaultAssay(filteredSeurat) <- "integrated"
filteredSeurat <- RunPCA(filteredSeurat, verbose=TRUE)
filteredSeurat <- FindNeighbors(filteredSeurat, dims=1:20, verbose=TRUE)
filteredSeurat <- FindClusters(filteredSeurat, verbose=TRUE, resolution=1)
filteredSeurat <- RunUMAP(filteredSeurat, dims=1:20)
