...
Male$Sex <- "MALE"
Female < CreateSeuratObject(counts = LigerFemale2a.data, project="LIGER", min.cells=3, min.features=200)
Female$Sex <- "FEMALE"
...
sex.anchors <- FindIntegrationAnchors(object.list=(Male,Female), dims=1:20)
...
DimPlot(sex.combined, reduction="umap", split.by="stim")
dev.off()
DefaultAssay(sex.combined) <- "RNA"
sex.markers <- FindConservedMarkers(sex.combined, indent.1=7, grouping.var = "Sex", verbose=TRUE)
head(sex.markers)
pdf(file="Seurat_Featureplot.pdf")
