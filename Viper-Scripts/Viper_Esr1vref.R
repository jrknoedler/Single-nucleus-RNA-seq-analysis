...
null_model <- viper::ttestNull(x=Esr1.mat, y=Ref.mat, per=500, repos=TRUE, verbose=T)
final_out <- viper::msviper(signatures, regul, null_model, verbose=T)
saveRDS(final_out, file="ViperOut/POA/Inhibitoryvres_OEinhouse.rds")
df_obj <- summary(final_out, length=(final_out$es$nes)) %>% as.data.frame() %>% arrange(FDR)
df_obj
pdf(file=paste0(output,"mrsplot1Esr1oeinhouse.pdf"))
